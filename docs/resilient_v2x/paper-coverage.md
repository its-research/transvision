...
当前已有一个单随机种子的 12-condition ClearML 证据包通过内容哈希校验，
但尚未达到至少 3 个随机种子及同协议基线齐备的论文提交门槛。因此，下面
的状态仍只回答“代码是否覆盖”；单种子诊断值不能升级为最终论文结论，也
不能替 checkpoint、日志、预测或 evaluator 输出生成结果。
