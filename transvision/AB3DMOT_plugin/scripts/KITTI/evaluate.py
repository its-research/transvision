...
def boxoverlap(a, b, criterion='union'):
    """boxoverlap computes intersection over union for bbox a and b in KITTI
    format.
...
class trackingEvaluation(object):
    """tracking statistics (CLEAR MOT, id-switches, fragments, ML/PT/MT,
    precision/recall)
    ...
    def __init__(self,
                 t_sha,
                 gt_path='AB3DMOT_plugin/scripts/KITTI',
                 max_truncation=0,
                 min_height=25,
                 max_occlusion=2,
                 mail=None,
        ...
        # print('min overlap criteria is %f' % self.min_overlap)
    ...
    def _loadData(self, root_dir, cls, min_score=-1000, loading_groundtruth=False):
        ...
        Use loadGroundtruth() or loadTracker() to load this data. Loads detections in KITTI format from textfiles.
        """
            ...
            for line in f:
                ...
                try:
                    ...
                    if id_frame in id_frame_cache and not loading_groundtruth:
                        ...
                        self.mail.msg('track id %d occurred at least twice for this frame' % t_data.track_id)
    ...
    def createSummary_simple(self, threshold, recall):
        ...
        summary += '{:.4f} {:.4f} {:.4f} {:.4f} {:.4f} {:5d} {:5d} {:.4f} {:.4f} {:.4f} {:.4f} {:5d} {:5d} {:5d}\n'.format(self.sMOTA, self.MOTA, self.MOTP, self.MT, self.ML,
                                                                                                                           self.id_switches, self.fragments, self.F1,
                                                                                                                           self.precision, self.recall, self.FAR, self.tp, self.fp,
                                                                                                                           self.fn)
        summary += '=' * 80
...
def evaluate(result_sha, mail, num_hypo, eval_3diou, eval_2diou, thres):
    ...
    # start evaluation and instantiated eval object
    if eval_3diou:
    ...
    for c in ['car']:
        ...
        for threshold_tmp, recall_tmp in zip(threshold_list, recall_list):
            ...
            data_tmp['mota'], data_tmp['motp'], data_tmp['moda'], data_tmp['modp'], data_tmp['precision'], data_tmp['F1'], data_tmp['fp'], data_tmp['fn'], data_tmp[
                'recall'], data_tmp['sMOTA'] = e.MOTA, e.MOTP, e.MODA, e.MODP, e.precision, e.F1, e.fp, e.fn, e.recall, e.sMOTA
