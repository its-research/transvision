...
from xinshuo_io import fileparts, is_path_exists, load_list_from_folder
...
def get_subfolder_seq(dataset, split, split_data_path):
    ...
    if dataset == 'KITTI':  # KITTI
        det_id2str = {0: 'Trafficcone', 1: 'Pedestrian', 2: 'Car', 3: 'Cyclist', 4: 'Van', 5: 'Truck', 6: 'Bus', 7: 'Tricyclist', 8: 'Motorcyclist', 9: 'Barrowlist'}
...
def get_threshold(dataset, det_name):
    ...
    if dataset == 'KITTI':
        ...
        if det_name == 'megvii':
            return {'Car': 0.262545, 'Pedestrian': 0.217600, 'Truck': 0.294967, 'Trailer': 0.292775, 'Bus': 0.440060, 'Motorcycle': 0.314693, 'Bicycle': 0.284720}
        if det_name == 'centerpoint':
            return {'Car': 0.269231, 'Pedestrian': 0.410000, 'Truck': 0.300000, 'Trailer': 0.372632, 'Bus': 0.430000, 'Motorcycle': 0.368667, 'Bicycle': 0.394146}
        else:
            assert False, 'error, detection method not supported for getting threshold' % det_name
    else:
...
def find_all_frames(root_dir, subset, data_suffix, seq_list):
    ...
    # return a dictionary with each key corresponds to the list of frame ID
