import numpy as np
from AB3DMOT_libs.dist_metrics import dist3d, dist_ground, iou, m_distance
    ...
    for d, det in enumerate(dets):
        for t, trk in enumerate(trks):
            try:
                ...
                if metric in ['giou_2d', 'giou_3d']:
                    dist_now = -1.0
                elif metric in ['iou_2d', 'iou_3d']:
                    dist_now = 0.0
                elif metric in ['dist_3d', 'dist_2d', 'm_dis']:
                    dist_now = -100
                else:
                    assert False, 'error'
...
def data_association(dets, trks, metric, threshold, algm='greedy', trk_innovation_matrix=None, hypothesis=1):
