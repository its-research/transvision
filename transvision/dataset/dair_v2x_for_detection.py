...
class VICDatasetSPD(DAIRV2XDataset):
    ...
    def __init__(self, path, args, split='train', sensortype='lidar', extended_range=None, val_data_path=''):
        ...
        for elem in frame_pairs:
            ...
            vic_frame = VICFrameSPD(path, elem, veh_frame, inf_frame, 0, elem['system_error_offset']['delta_x'], elem['system_error_offset']['delta_y'])
...
class VICAsyncDatasetSPD(VICDatasetSPD):
    ...
    def prev_inf_frame(self, index, sensortype='lidar'):
        if sensortype == 'lidar':
            ...
            if (int(index) - self.k < int(cur['valid_frames_splits'][0]['start_frame_id']) or id_to_str(int(index) - self.k) not in self.inf_frame2info.keys()):
                return None, None
