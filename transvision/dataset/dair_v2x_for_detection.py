...
class DAIRV2XI(DAIRV2XDataset):
    ...
    def __init__(self,
        ...
        data_infos = load_json(
            osp.join(path, 'infrastructure-side/data_info.json'))
        ...
        for elem in data_infos:
            gt_label = {}
            filt = RectFilter(
                extended_range[0]) if extended_range is not None else Filter()
            gt_label['camera'] = Label(
                osp.join(path, 'infrastructure-side',
                         elem['label_camera_std_path']), filt)
            gt_label['lidar'] = Label(
                osp.join(path, 'infrastructure-side',
                         elem['label_lidar_std_path']), filt)

            self.data.append((InfFrame(path, elem), gt_label, filt))

            if sensortype == 'camera':
                inf_frame = self.inf_path2info[elem['image_path']]
                get_annos(path + '/infrastructure-side', '', inf_frame,
                          'camera')
    ...
    def get_split(self, split_path, split, data_infos):
        ...
        for data_info in data_infos:
            frame_idx = data_info['image_path'].split('/')[-1].replace(
                '.jpg', '')
    ...
    def __init__(self,
        ...
        for elem in data_infos:
            ...
            for view in ['camera', 'lidar']:
                gt_label[view] = Label(
                    osp.join(path, 'vehicle-side',
                             elem['label_' + view + '_std_path']), filt)
...
class VICDataset(DAIRV2XDataset):
    ...
    def __init__(self,
        ...
        if args.model == 'feature_flow':
            ...
            frame_pairs = load_json(
                osp.join(path, 'cooperative/data_info.json'))
            split_path = args.split_data_path
        ...
        for elem in frame_pairs:
            if sensortype == 'lidar':
                inf_frame = self.inf_path2info[
                    elem['infrastructure_pointcloud_path']]
                veh_frame = self.veh_path2info[elem['vehicle_pointcloud_path']]
            elif sensortype == 'camera':
                inf_frame = self.inf_path2info[
                    elem['infrastructure_image_path']]
            ...
            if extended_range is not None:
                trans = vic_frame.transform(
                    from_coord='Vehicle_lidar', to_coord='World')
                filt_world = RectFilter(trans(extended_range)[0])

            trans_1 = vic_frame.transform('World', 'Vehicle_lidar')
            label_v = Label(
                osp.join(path, elem['cooperative_label_path']), filt_world)
    ...
    def query_veh_segment(self,
        ...
        return [
            f for f in segment
            if f.id[sensortype] < frame.id[sensortype] or not previous_only
        ]

    def query_inf_segment(self,
                          frame,
                          sensortype='lidar',
                          previous_only=False):
    ...
    def get_split(self, split_path, split, frame_pairs):
        ...
        for frame_pair in frame_pairs:
            veh_frame_idx = frame_pair['vehicle_image_path'].split(
                '/')[-1].replace('.jpg', '')
...
class VICSyncDataset(VICDataset):
    ...
    def __init__(self,
        ...
        super().__init__(path, args, split, sensortype, extended_range,
                         val_data_path)
        logger.info('VIC-Sync {} dataset, overall {} frames'.format(
            split, len(self.data)))
...
class VICAsyncDataset(VICDataset):
    ...
    def __init__(self,
        ...
        for vic_frame, coop_labels, filt in self.data:
            ...
            if inf_frame is None:
                ...
                new_vic_frame = VICFrame(path, {}, vic_frame.veh_frame,
                                         inf_frame, delta_t, vic_frame.offset)
                self.async_data.append((new_vic_frame, coop_labels, filt))

        logger.info('VIC-Async {} dataset, overall {} frames'.format(
            split, len(self.async_data)))
    ...
    def prev_inf_frame(self, index, sensortype='lidar'):
        if sensortype == 'lidar':
            cur = self.inf_path2info['infrastructure-side/velodyne/' + index +
                                     '.pcd']
            if int(index) - self.k < int(
                    cur['batch_start_id']
            ) or 'infrastructure-side/velodyne/' + id_to_str(
                    int(index) - self.k) + '.pcd' not in self.inf_path2info:
                return None, None
            prev = self.inf_path2info['infrastructure-side/velodyne/' +
                                      id_to_str(int(index) - self.k) + '.pcd']
            return (
                InfFrame(self.path + '/infrastructure-side/', prev),
                (int(cur['pointcloud_timestamp']) -
                 int(prev['pointcloud_timestamp'])) / 1000.0,
            )
        elif sensortype == 'camera':
            cur = self.inf_path2info['infrastructure-side/image/' + index +
                                     '.jpg']
            if int(index) - self.k < int(cur['batch_start_id']):
                return None, None
            prev = self.inf_path2info['infrastructure-side/image/' +
                                      id_to_str(int(index) - self.k) + '.jpg']
            get_annos(self.path, 'infrastructure-side', prev, 'camera')
            return (
                InfFrame(self.path + '/infrastructure-side/', prev),
                (int(cur['image_timestamp']) - int(prev['image_timestamp'])) /
                1000.0,
            )
...
if __name__ == '__main__':
    ...
    extended_range = np.array([[box_range[index] for index in indexes]])
    dataset = VICSyncDataset(
        input, split, sensortype, extended_range=extended_range)

    for VICFrame_data, _label, _filt in tqdm(dataset):
        veh_image_path = VICFrame_data.vehicle_frame()['image_path'][-10:-4]
        inf_image_path = VICFrame_data.infrastructure_frame(
        )['image_path'][-10:-4]
