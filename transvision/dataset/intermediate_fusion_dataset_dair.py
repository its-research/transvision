...
from opencood.utils.transformation_utils import inf_side_rot_and_trans_to_trasnformation_matrix, tfm_to_pose, veh_side_rot_and_trans_to_trasnformation_matrix, x1_to_x2, x_to_world
...
class IntermediateFusionDatasetDAIR(Dataset):
    ...
    def __init__(self, params, visualize, train="train"):
        self.params = params
        self.visualize = visualize
        self.train = train

        if self.train == "train":
            self.data_augmentor = DataAugmentor(params["data_augment"], True)
        else:
            self.data_augmentor = DataAugmentor(params["data_augment"], False)
        self.max_cav = 2
