...
from transvision.pcdet_utils.iou3d_nms import iou3d_nms_cuda
from transvision.utils.common_utils import check_numpy_to_torch
...
def decode_boxes_and_giou3d(boxes_a, boxes_b, pc_range, box_mean, box_std):
    ...
    lwh = torch.max(corners_a.max(dim=1)[0][:, None, :], corners_b.max(dim=1)[0]) - torch.min(corners_a.min(dim=1)[0][:, None, :], corners_b.min(dim=1)[0])
...
def _centroid_to_corners_torch(boxes):
    ...
    corners[:, :, 2] = torch.cat([torch.stack([boxes[:, 2] - boxes[:, 5] / 2] * 4, dim=1), torch.stack([boxes[:, 2] + boxes[:, 5] / 2] * 4, dim=1)], dim=1)
...
def _centroid_to_corners_np(boxes):
    ...
    corners[:, :, 2] = np.concatenate([np.stack([boxes[:, 2] - boxes[:, 5] / 2] * 4, axis=1), np.stack([boxes[:, 2] + boxes[:, 5] / 2] * 4, axis=1)], axis=1)
