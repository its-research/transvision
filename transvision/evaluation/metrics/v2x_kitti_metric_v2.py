...
from mmdet3d.structures import Box3DMode, CameraInstance3DBoxes, LiDARInstance3DBoxes, limit_period, points_cam2img
...
class V2XKittiMetricV2(BaseMetric):
    ...
    def __init__(self,
        ...
        super(V2XKittiMetricV2, self).__init__(collect_device=collect_device, prefix=prefix)
    ...
    def convert_annos_to_kitti_annos(self, data_infos: dict) -> List[dict]:
        ...
        for i, data_anno in enumerate(data_annos):

            anno_path = os.path.join(self.data_root, data_anno['label_lidar_std_path'])
            ...
            kitti_annos['alpha'] = []
            kitti_annos['bbox'] = []
            calib_v_lidar2cam_filename = os.path.join(self.data_root, data_annos[i]['calib_lidar_to_camera_path'])
            calib_v_lidar2cam = load(calib_v_lidar2cam_filename)
            calib_v_cam_intrinsic_filename = os.path.join(self.data_root, data_annos[i]['calib_camera_intrinsic_path'])
            ...
            data_annos[i]['calib'] = {}
            data_annos[i]['calib']['R0_rect'] = rect
            data_annos[i]['calib']['Tr_velo_to_cam'] = Trv2c
            data_annos[i]['calib']['P2'] = P2
    ...
    def compute_metrics(self, results: List[dict]) -> Dict[str, float]:
        ...
        result_dict, tmp_dir = self.format_results(results, pklfile_prefix=self.pklfile_prefix, submission_prefix=self.submission_prefix, classes=self.classes)
        ...
        # print(gt_annos[728])
        # print(result_dict['pred_instances_3d'][728])

        for metric in self.metrics:
            ap_dict = self.kitti_evaluate(result_dict, gt_annos, metric=metric, logger=logger, classes=self.classes)
    ...
    def kitti_evaluate(self,
        ...
        for name in results_dict:
            ...
            ap_result_str, ap_dict_ = kitti_eval(gt_annos, results_dict[name], classes, eval_types=eval_types)
    ...
    def format_results(self,
                       ...
                       pklfile_prefix: Optional[str] = None,
                       submission_prefix: Optional[str] = None,
                       classes: Optional[List[str]] = None) -> Tuple[dict, Union[tempfile.TemporaryDirectory, None]]:
        ...
        for name in results[0]:
            ...
            if 'pred_instances' in name and '3d' in name and name[0] != '_' and results[0][name]:
                ...
                result_list_ = self.bbox2result_kitti(net_outputs, sample_idx_list, classes, pklfile_prefix_, submission_prefix_)
                result_dict[name] = result_list_
            elif name == 'pred_instances' and name[0] != '_' and results[0][name]:
                net_outputs = [result[name] for result in results]
                result_list_ = self.bbox2result_kitti2d(net_outputs, sample_idx_list, classes, pklfile_prefix_, submission_prefix_)
                result_dict[name] = result_list_
    ...
    def bbox2result_kitti2d(self,
        ...
        for i, bboxes_per_sample in enumerate(mmengine.track_iter_progress(net_outputs)):
