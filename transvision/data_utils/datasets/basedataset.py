...
from transvision.hypes_yaml.yaml_utils import load_yaml
from transvision.utils.keypoint_utils import bev_sample
from transvision.utils.pcd_utils import downsample_lidar_minimum
...
class BaseDataset(Dataset):
    ...
    pre_processor : transvision.pre_processor
        Used to preprocess the raw data.

    post_processor : transvision.post_processor
        Used to generate training labels and convert the model outputs to
        bbx formats.

    data_augmentor : transvision.data_augmentor
                        ...
                        if (self.proj_first and j == 0) or (not self.proj_first):
                            ...
                            if os.path.exists(kp_file_path):
                                ...
                                bev_keypoints = bev_sample(
                                    pcd_np, self.params["select_kp"]["lidar_range"], self.params["select_kp"]["n_samples"], mode="uniform", all_samples=False
                                )
