...
from transvision.utils.transformation_utils import inf_side_rot_and_trans_to_trasnformation_matrix, tfm_to_pose, veh_side_rot_and_trans_to_trasnformation_matrix, x1_to_x2
...
class LateFusionDatasetDAIR(Dataset):
    ...
    def get_item_single_car(self, selected_cav_base):
        ...
        object_bbx_center, object_bbx_mask, object_ids = self.generate_object_center([selected_cav_base], selected_cav_base["params"]["lidar_pose_clean"])
    ...
    def get_item_test(self, base_data_dict, idx):
        ...
        for cav_id in cav_id_list:
            ...
            selected_cav_processed = self.get_item_single_car(selected_cav_base)
            selected_cav_processed.update({"transformation_matrix": transformation_matrix, "transformation_matrix_clean": transformation_matrix_clean})
    ...
    def collate_batch_test(self, batch):
        ...
        if self.visualize:
            ...
            output_dict["ego"].update({"origin_lidar_v": [torch.from_numpy(projected_lidar_list[0])]})
            output_dict["ego"].update({"origin_lidar_i": [torch.from_numpy(projected_lidar_list[1])]})
