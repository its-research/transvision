...
def quaternion_trans(input_point, translation, rotation):
    ...
    input = np.concatenate(
        (np.array(input_point), np.array([1]))).reshape(4, 1)
...
def box_translation(boxes, translation, rotation):
    ...
    for i in range(n):
        for j in range(c):
            result[i, j, :] = quaternion_trans(boxes[i, j, :], translation,
                                               rotation)
    return result
...
def get_arrow_end(centers, angles, vector=[5, 5]):
    ...
    for angle in angles:
            ...
            [vector[0] * math.sin(angle), vector[1] * math.cos(angle), 0])
...
class Coord_transformation(object):
    """coord_list=['Infrastructure_image','Infrastructure_camera','Infrastructu
    re_lidar', 'world', 'Vehicle_image','Vehicle_camera','Vehicle_lidar',
    'Vehicle_novatel']
    ...
    def __call__(self, point):
        path_all = {
            ...
            self.infra_name + '.json',
            'path_lidar2novatel':
            'vehicle-side/calib/lidar_to_novatel/' + self.veh_name + '.json',
            'path_novatel2world':
            'vehicle-side/calib/novatel_to_world/' + self.veh_name + '.json',
        }

        rotation, translation = self.forward(self.from_coord, self.to_coord,
                                             path_all)
    ...
    def forward(self, from_coord, to_coord, path_all):
        ...
        if (from_coord in coord_list) and (to_coord in coord_list):
            if from_coord == 'Infrastructure_lidar' and to_coord == 'World':
                rotation, translation = self.Coord_Infrastructure_lidar2world(
                    path_all)
                return rotation, translation
            if from_coord == 'Vehicle_lidar' and to_coord == 'World':
                rotation, translation = self.Coord_Vehicle_lidar2world(
                    path_all)
                return rotation, translation
            ...
            if from_coord == 'World' and to_coord == 'Vehicle_lidar':
                rotation, translation = self.Coord_world2vehicel_lidar(
                    path_all)
                return rotation, translation
        else:
            raise Exception('error: wrong coordinate name')
    ...
    def muilt_coord(self, rotationA2B, translationA2B, rotationB2C,
                    translationB2C):
    ...
    def trans(self, input_point, translation, rotation):
        ...
        for _point in input_point:
            output_point = np.dot(rotation, input_point.reshape(
                3, 1)).reshape(3) + np.array(translation).reshape(3)
    ...
    def get_lidar2world(
            self, path_lidar2world):  # Infrastructure side, lidar to word
    ...
    def Coord_Infrastructure_lidar2Vehicle_lidar(self, path_all):
        ...
        translationA2B = translationA2B + np.array([delta_x, delta_y, 0
                                                    ]).reshape(3, 1)
...
if __name__ == '__main__':
    ...
    translation = [[-5.779144404715124], [6.037615758600886],
                   [1.0636424034755758]]
    ...
    translation_other = [[3390.138583273976], [2087.3119082041085],
                         [20.66834816604844]]
