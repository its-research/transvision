...
def range2box(box_range):
    ...
    return np.array([[box_range[index] for index in indices]])
...
def above_line(point, st, ed):
    ...
    if abs(b) > 1e-6 and abs(a) > 1e-6:
        ...
        return (y_intersec >= st[1] and y_intersec <= ed[1] or y_intersec >= ed[1] and y_intersec <= st[1]) and y_intersec < point[1]
    elif abs(b) > 1e-6:
        return (point[0] >= st[0] and point[0] <= ed[0] or point[0] >= ed[0] and point[0] <= st[0]) and point[1] >= st[1]
    else:
        return 0
...
def point_in_matrix(point, matrix):
    point
    return (above_line(point, matrix[0], matrix[1]) + above_line(point, matrix[1], matrix[2]) + above_line(point, matrix[2], matrix[3]) +
            above_line(point, matrix[3], matrix[0])) % 2 == 1
...
def above_plane(point, plane):
    ...
    t = (norm[0] * point[0] + norm[1] * point[1] + norm[2] * point[2] - d) / (norm[0]**2 + norm[1]**2 + norm[2]**2)
