...
def above_line(point, st, ed):
    ...
    if abs(b) > 1e-6 and abs(a) > 1e-6:
        ...
        return (y_intersec >= st[1] and y_intersec <= ed[1]
                or y_intersec >= ed[1]
                and y_intersec <= st[1]) and y_intersec < point[1]
    elif abs(b) > 1e-6:
        return (point[0] >= st[0] and point[0] <= ed[0] or point[0] >= ed[0]
                and point[0] <= st[0]) and point[1] >= st[1]
    else:
        return 0
...
def point_in_matrix(point, matrix):
    point
    return (above_line(point, matrix[0], matrix[1]) +
...
def isInSide(x1, y1, x2, y2, x3, y3, x4, y4, x, y):
    return GetCross(x1, y1, x2, y2, x, y) * GetCross(
        x3, y3, x4, y4, x, y) >= 0 and GetCross(
            x2, y2, x3, y3, x, y) * GetCross(x4, y4, x1, y1, x, y) >= 0
