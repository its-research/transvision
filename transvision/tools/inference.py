...
from transvision.tools import inference_utils, train_utils
from transvision.utils import eval_utils
from transvision.visualization import simple_vis
...
def test_parser():
    ...
    parser.add_argument("--save_vis_n", type=int, default=10, help="save how many numbers of visualization result?")
    parser.add_argument("--save_npy", action="store_true", help="whether to save prediction and gt result" "in npy file")
    parser.add_argument("--eval_epoch", type=str, default=None, help="Set the checkpoint")
...
def main():
    ...
    transvision_dataset = build_dataset(hypes, visualize=True, train=False)
    data_loader = DataLoader(transvision_dataset, batch_size=1, num_workers=4, collate_fn=transvision_dataset.collate_batch_test, shuffle=False, pin_memory=False, drop_last=False)
    ...
    for i, batch_data in tqdm(enumerate(data_loader)):
        with torch.no_grad():
            ...
            if opt.fusion_method == "late":
                pred_box_tensor, pred_score, gt_box_tensor = inference_utils.inference_late_fusion(batch_data, model, transvision_dataset)
            elif opt.fusion_method == "early":
                pred_box_tensor, pred_score, gt_box_tensor = inference_utils.inference_early_fusion(batch_data, model, transvision_dataset)
            elif opt.fusion_method == "intermediate":
                pred_box_tensor, pred_score, gt_box_tensor = inference_utils.inference_intermediate_fusion(batch_data, model, transvision_dataset)
            elif opt.fusion_method == "no":
                pred_box_tensor, pred_score, gt_box_tensor = inference_utils.inference_no_fusion(batch_data, model, transvision_dataset)

            elif opt.fusion_method == "intermediate_with_comm":
                pred_box_tensor, pred_score, gt_box_tensor, comm_rates = inference_utils.inference_intermediate_fusion_withcomm(batch_data, model, transvision_dataset)
