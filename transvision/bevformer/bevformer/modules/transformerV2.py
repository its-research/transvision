...
from mmcv.cnn import build_conv_layer, build_norm_layer, xavier_init
...
class ResNetFusion(BaseModule):
    ...
    def __init__(self, in_channels, out_channels, inter_channels, num_layer, norm_cfg=dict(type='SyncBN'), with_cp=False):
            ...
            if i == 0:
                if inter_channels == in_channels:
                    ...
                    downsample = nn.Sequential(
                        build_conv_layer(None, in_channels, inter_channels, 3, stride=1, padding=1, dilation=1, bias=False),
                        build_norm_layer(norm_cfg, inter_channels)[1])
                    layers.append(BasicBlock(in_channels, inter_channels, stride=1, norm_cfg=norm_cfg, downsample=downsample))
            else:
...
class PerceptionTransformerBEVEncoder(BaseModule):
    ...
    def __init__(self, num_feature_levels=4, num_cams=6, two_stage_num_proposals=300, encoder=None, embed_dims=256, use_cams_embeds=True, rotate_center=[100, 100], **kwargs):
    ...
    def forward(self, mlvl_feats, bev_queries, bev_h, bev_w, grid_length=[0.512, 0.512], bev_pos=None, prev_bev=None, **kwargs):
        """obtain bev features."""
        ...
        if 'aug_param' in kwargs['img_metas'][0] and 'GlobalRotScaleTransImage_param' in kwargs['img_metas'][0]['aug_param']:
            rot_angle, scale_ratio, flip_dx, flip_dy, bda_mat, only_gt = kwargs['img_metas'][0]['aug_param']['GlobalRotScaleTransImage_param']
            ...
            if only_gt:
                ...
                bda_mat = bda_mat[:2, :2].to(grid_shift).view(1, 1, 1, 2, 2).repeat(grid_shift.shape[0], grid_shift.shape[1], grid_shift.shape[2], 1, 1)
...
class PerceptionTransformerV2(PerceptionTransformerBEVEncoder):
    ...
    def __init__(self,
                 ...
                 inter_channels=None,
                 **kwargs):
        super(PerceptionTransformerV2, self).__init__(num_feature_levels, num_cams, two_stage_num_proposals, encoder, embed_dims, use_cams_embeds, rotate_center, **kwargs)
        ...
        if len(self.frames) > 1:
            self.fusion = ResNetFusion(
                len(self.frames) * self.embed_dims, self.embed_dims, inter_channels if inter_channels is not None else len(self.frames) * self.embed_dims, num_fusion)
    ...
    def forward(self,
        ...
        bev_embed = self.get_bev_features(
            mlvl_feats, bev_queries, bev_h, bev_w, grid_length=grid_length, bev_pos=bev_pos, prev_bev=None, **kwargs)  # bev_embed shape: bs, bev_h*bev_w, embed_dims
