...
from transvision.bevformer.core.evaluation.eval_hooks import CustomDistEvalHook
from transvision.bevformer.datasets import custom_build_dataset
from transvision.bevformer.datasets.builder import build_dataloader
...
def custom_train_detector(model, dataset, cfg, distributed=False, validate=False, timestamp=None, eval_model=None, meta=None):
    ...
    if distributed:
        ...
        if eval_model is not None:
            eval_model = MMDistributedDataParallel(
                eval_model.cuda(), device_ids=[torch.cuda.current_device()], broadcast_buffers=False, find_unused_parameters=find_unused_parameters)
    else:
    ...
    if eval_model is not None:
        runner = build_runner(cfg.runner, default_args=dict(model=model, eval_model=eval_model, optimizer=optimizer, work_dir=cfg.work_dir, logger=logger, meta=meta))
    else:
