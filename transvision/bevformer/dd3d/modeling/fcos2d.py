...
from transvision.bevformer.dd3d.layers.iou_loss import IOULoss
from transvision.bevformer.dd3d.layers.normalization import ModuleListDial, Scale
from transvision.bevformer.dd3d.utils.comm import reduce_sum
...
class FCOS2DHead(nn.Module):
    ...
    def __init__(self, num_classes, input_shape, num_cls_convs=4, num_box_convs=4, norm='BN', use_deformable=False, use_scale=True, box2d_scale_init_factor=1.0, version='v2'):
        super().__init__()
        ...
        for head_name, num_convs in head_configs.items():
            ...
            if self._version == 'v1':
                ...
                for _ in range(num_convs):
                    ...
                    tower.append(Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1, bias=norm_layer is None, norm=norm_layer, activation=F.relu))
            else:
        ...
        if self.use_scale:
            if self._version == 'v1':
                self.scales_reg = nn.ModuleList([Scale(init_value=stride * self.box2d_scale_init_factor) for stride in self.in_strides])
            else:
                self.scales_box2d_reg = nn.ModuleList([Scale(init_value=stride * self.box2d_scale_init_factor) for stride in self.in_strides])
...
class FCOS2DLoss(nn.Module):
    ...
    def forward(self, logits, box2d_reg, centerness, targets):
        ...
        if len(labels) != box2d_reg_targets.shape[0]:
            raise ValueError(f"The size of 'labels' and 'box2d_reg_targets' does not match: a={len(labels)}, b={box2d_reg_targets.shape[0]}")
        ...
        # Centerness loss
        # ---------------
        loss_centerness = F.binary_cross_entropy_with_logits(centerness_pred, centerness_targets, reduction='sum') / num_pos_avg
...
class FCOS2DInference():
    ...
    def __call__(self, logits, box2d_reg, centerness, locations, image_sizes):
        ...
        extra_info = []
        for lvl, (logits_lvl, box2d_reg_lvl, centerness_lvl, locations_lvl) in enumerate(zip(logits, box2d_reg, centerness, locations)):

            instances_per_lvl, extra_info_per_lvl = self.forward_for_single_feature_map(logits_lvl, box2d_reg_lvl, centerness_lvl, locations_lvl,
                                                                                        image_sizes)  # List of Instances; one for each image.
    ...
    def forward_for_single_feature_map(self, logits, box2d_reg, centerness, locations, image_sizes):
        ...
        extra_info = {'fg_inds_per_im': all_fg_inds_per_im, 'class_inds_per_im': all_class_inds_per_im, 'topk_indices': all_topk_indices}
    ...
    def nms_and_top_k(self, instances_per_im, score_key_for_nms='scores'):
        ...
        for instances in instances_per_im:
            if self.nms_thresh > 0:
                # Multiclass NMS.
                keep = batched_nms(instances.pred_boxes.tensor, instances.get(score_key_for_nms), instances.pred_classes, self.nms_thresh)
