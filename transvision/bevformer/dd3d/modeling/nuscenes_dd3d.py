...
from transvision.bevformer.dd3d.datasets.nuscenes import MAX_NUM_ATTRIBUTES
...
class NuscenesDD3DTargetPreparer(DD3DTargetPreparer):
    ...
    def __call__(self, locations, gt_instances, feature_shapes):
        ...
        training_targets['fpn_levels'] = [loc.new_ones(len(loc), dtype=torch.long) * level for level, loc in enumerate(training_targets['locations'])]
...
class NuscenesInference():
    ...
    def __call__(self, attr_logits, speeds, pred_instances, fcos2d_info):
        ...
        for lvl, (attr_logits_lvl, speed_lvl, info_lvl, instances_lvl) in enumerate(zip(attr_logits, speeds, fcos2d_info, pred_instances)):
...
class NuscenesDD3D(DD3D):
    ...
    def __init__(self,
        ...
        self.prepare_targets = NuscenesDD3DTargetPreparer(num_classes=num_classes, input_shape=self.backbone_output_shape, box3d_on=box3d_on, **target_assign_cfg)
    ...
    def forward(self, features, batched_inputs):
        ...
        if self.training:
            ...
            if not self.only_box2d:
                fcos3d_loss = self.fcos3d_loss(box3d_quat, box3d_ctr, box3d_depth, box3d_size, box3d_conf, dense_depth, inv_intrinsics, fcos2d_info, training_targets)
            ...
            #     self.fcos3d_inference(box3d_quat, box3d_ctr, box3d_depth, box3d_size, box3d_conf, inv_intrinsics, pred_instances, fcos2d_info)
            #     score_key = 'scores_3d'
            # else:
            #     score_key = 'scores'
            ...
            #         pred_instances, dummy_group_idxs, self.num_classes, poses, iou_threshold=self.bev_nms_iou_thresh, include_boxes3d_global=False)
            ...
            #         instances, group_idxs, self.num_classes, global_poses, self.bev_nms_iou_thresh, max_num_dets_per_sample=self.max_num_dets_per_sample)
