...
from transvision.bevformer.dd3d.layers.normalization import ModuleListDial, Offset, Scale
from transvision.bevformer.dd3d.structures.boxes3d import Boxes3D
from transvision.bevformer.dd3d.utils.geometry import allocentric_to_egocentric, unproject_points2d
...
class FCOS3DHead(nn.Module):
    ...
    def __init__(
        ...
        # 3D box branches.
        self.box3d_quat = nn.ModuleList([Conv2d(in_channels, 4 * num_classes, kernel_size=3, stride=1, padding=1, bias=True) for _ in range(num_levels)])
        self.box3d_ctr = nn.ModuleList([Conv2d(in_channels, 2 * num_classes, kernel_size=3, stride=1, padding=1, bias=True) for _ in range(num_levels)])
        self.box3d_depth = nn.ModuleList([Conv2d(in_channels, 1 * num_classes, kernel_size=3, stride=1, padding=1, bias=(not self.use_scale)) for _ in range(num_levels)])
        self.box3d_size = nn.ModuleList([Conv2d(in_channels, 3 * num_classes, kernel_size=3, stride=1, padding=1, bias=True) for _ in range(num_levels)])
        self.box3d_conf = nn.ModuleList([Conv2d(in_channels, 1 * num_classes, kernel_size=3, stride=1, padding=1, bias=True) for _ in range(num_levels)])

        if self.use_scale:
            self.scales_proj_ctr = nn.ModuleList([Scale(init_value=stride * self.proj_ctr_scale_init_factor) for stride in self.in_strides])
            ...
            self.scales_depth = nn.ModuleList([Scale(init_value=sigma * self.depth_scale_init_factor) for sigma in self.std_depth_per_level])
...
class FCOS3DLoss(nn.Module):
    ...
    def forward(self, box3d_quat, box3d_ctr, box3d_depth, box3d_size, box3d_conf, dense_depth, inv_intrinsics, fcos2d_info, targets):
...
class FCOS3DInference():
    ...
    def __call__(self, box3d_quat, box3d_ctr, box3d_depth, box3d_size, box3d_conf, inv_intrinsics, pred_instances, fcos2d_info):
        # pred_instances: # List[List[Instances]], shape = (L, B)
        for lvl, (box3d_quat_lvl, box3d_ctr_lvl, box3d_depth_lvl, box3d_size_lvl, box3d_conf_lvl) in enumerate(zip(box3d_quat, box3d_ctr, box3d_depth, box3d_size, box3d_conf)):

            # In-place modification: update per-level pred_instances.
            self.forward_for_single_feature_map(box3d_quat_lvl, box3d_ctr_lvl, box3d_depth_lvl, box3d_size_lvl, box3d_conf_lvl, inv_intrinsics, pred_instances[lvl],
                                                fcos2d_info[lvl])  # List of Instances; one for each image.

    def forward_for_single_feature_map(self, box3d_quat, box3d_ctr, box3d_depth, box3d_size, box3d_conf, inv_intrinsics, pred_instances, fcos2d_info):
