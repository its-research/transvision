...
def _as_tensor(x: Tuple[int, int]) -> torch.Tensor:
    ...
    `torch.as_tensor` will record a constant in tracing, but this function will use `torch.stack` instead.
    """
...
class ImageList(object):
    ...
    blob/master/detectron2/structures/image_list.py)
    ...
    @staticmethod
    def from_tensors(tensors: List[torch.Tensor], size_divisibility: int = 0, pad_value: float = 0.0, intrinsics=None, image_paths=None) -> 'ImageList':
        """
        Args:
