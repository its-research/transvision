...
import transvision.bevformer.dd3d.structures.transform3d as t3d
from transvision.bevformer.dd3d.utils.geometry import unproject_points2d
...
class Boxes3D(GenericBoxes3D):
    ...
    def __getitem__(self, item):
        """"""
        if isinstance(item, int):
            return Boxes3D(self.quat[item].view(1, -1), self.proj_ctr[item].view(1, -1), self.depth[item].view(1, -1), self.size[item].view(1, -1),
    ...
    def clone(self):
        """"""
        return Boxes3D(self.quat.clone(), self.proj_ctr.clone(), self.depth.clone(), self.size.clone(), self.inv_intrinsics.clone())
