...
from transvision.bevformer.dd3d.structures.boxes3d import GenericBoxes3D
from transvision.bevformer.dd3d.structures.pose import Pose
from transvision.bevformer.dd3d.utils.geometry import project_points3d
from transvision.bevformer.dd3d.utils.visualization import float_to_uint8_color
...
class NuscenesDataset(Dataset):
    ...
    def get_instance_annotations(self, annotation_list, K, image_shape, pose_WS):
        ...
        for _ann in annotation_list:
            ...
            assert len(attr_tokens) < 2  # NOTE: Allow only single attribute.
    ...
    def __getitem__(self, idx):
        ...
        d2_dict = OrderedDict(file_name=filename, height=height, width=width, image_id=image_id, sample_id=sample_id, sample_token=sample_token)
    ...
    def getitem_by_datumtoken(self, datum_token):
        ...
        filename, _annotations, K = self.nusc.get_sample_data(datum_token)
        height, width = datum['height'], datum['width']
        d2_dict = OrderedDict(file_name=filename, height=height, width=width, image_id=0, sample_id=0, sample_token=sample_token)
        # Intrinsics
        d2_dict['intrinsics'] = list(K.flatten())
        # Get pose of the sensor (S) from vehicle (V) frame
        _pose_VS = self.nusc.get('calibrated_sensor', datum['calibrated_sensor_token'])
