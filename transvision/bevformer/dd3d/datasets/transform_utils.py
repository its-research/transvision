...
def transform_instance_annotations(
    ...
    /detectron2/data/detection_utils.py#L254.
...
def annotations_to_instances(
    ...
    if len(annos) and 'bbox3d' in annos[0]:
        ...
        if len(target.gt_boxes3d) != target.gt_boxes.tensor.shape[0]:
            raise ValueError(f'The sizes of `gt_boxes3d` and `gt_boxes` do not match: a={len(target.gt_boxes3d)}, b={target.gt_boxes.tensor.shape[0]}.')
