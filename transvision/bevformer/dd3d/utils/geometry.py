...
def allocentric_to_egocentric(quat, proj_ctr, inv_intrinsics):
    ...
    # ray == z-axis in local orientation
    ...
    if not torch.allclose(quat_norm, torch.as_tensor(1.), atol=1e-3):
        LOG.warning(f'Some of the input quaternions are not unit norm: min={quat_norm.min()}, max={quat_norm.max()}; therefore normalizing.')
...
def project_points3d(Xw, K):
    _, C = Xw.shape
    assert C == 3
    uv, _ = cv2.projectPoints(Xw, np.zeros((3, 1), dtype=np.float32), np.zeros(3, dtype=np.float32), K, np.zeros(5, dtype=np.float32))
