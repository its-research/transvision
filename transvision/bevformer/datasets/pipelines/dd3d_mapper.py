...
from transvision.bevformer.dd3d.datasets.transform_utils import annotations_to_instances
from transvision.bevformer.dd3d.structures.pose import Pose
from transvision.bevformer.dd3d.utils.tasks import TaskManager
    ...
    def __call__(self, results):
        ...
        for dataset_dict in results['mono_input_dict']:
            ...
            if 'extrinsics' in dataset_dict:
                extrinsics = Pose(wxyz=np.float32(dataset_dict['extrinsics']['wxyz']), tvec=np.float32(dataset_dict['extrinsics']['tvec']))
