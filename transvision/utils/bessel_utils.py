...
def logbessel_I_asymptotic(nu, z):
    ...
    result[z <= nu] = nu * torch.log(z[z <= nu]) + eta * z[z <= nu] - (nu + eta) * torch.log(torch.tensor(2.0)) - torch.log(torch.tensor(special.gamma(nu + 1)))
...
def logbessel_I(nu, z, fast=False, check=True):
    ...
    if check:
        ...
        # logbessel_approx
        assert len(result[torch.isnan(result)]) == 0, "Bessel functions take " + "NaN value for z = {}".format(z[torch.isnan(result)])
        assert len(result[torch.isinf(result)]) == 0, "Bessel functions take " + "inf value for z = {}".format(z[torch.isinf(result)])
    return result
