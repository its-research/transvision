...
class V2XVoxelNet(SingleStage3DDetector):
    ...
    def points_grid(self, points, point_cloud_range, voxel_size,
                    feature_shape):
        ...
        """
        origin_size_x = (point_cloud_range[3] -
                         point_cloud_range[0]) / voxel_size[0]
        ...
        points_grids[:, 1] = torch.round(
            (points[:, 1] - range_start_y) / voxel_size_y)
        points_grids[:, 2] = torch.round(
            (points[:, 0] - range_start_x) / voxel_size_x)
    ...
    def forward_train(
        ...
        feat_fused = self.feature_fusion(
            feat_veh, feat_inf, img_metas, mode='fusion')
    ...
    def simple_test(self,
        ...
        if len(bbox_list[0]
               [0].tensor) == 0:  # return a zero list when pre is empty
