...
def init_model(config: Union[str, Path, Config], checkpoint: Optional[str] = None, device: str = 'cuda:0', palette: str = 'none', cfg_options: Optional[dict] = None):
    ...
    if checkpoint is not None:
        ...
        if cfg_palette is not None:
            ...
            if 'palette' not in model.dataset_meta:
                warnings.warn('palette does not exist, random is used by default. '
...
def inference_multi_modality_detector(model: nn.Module,
    ...
    for index, pcd in enumerate(pcds):
        ...
        if cam_type != 'all':
            ...
            data_ = dict(lidar_points=dict(lidar_path=pcd), img_path=img, box_type_3d=box_type_3d, box_mode_3d=box_mode_3d)
            ...
            data_ = dict(lidar_points=dict(lidar_path=pcd), images=data_info['images'], box_type_3d=box_type_3d, box_mode_3d=box_mode_3d)
