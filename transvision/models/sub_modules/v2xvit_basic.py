...
from transvision.models.sub_modules.base_transformer import CavAttention, FeedForward, PreNorm, PyramidWindowAttention
from transvision.models.sub_modules.hmsa import HGTCavAttention
from transvision.models.sub_modules.mswin import PyramidWindowAttention
from transvision.models.sub_modules.torch_transformation_utils import get_discretized_transformation_matrix, get_roi_and_cav_mask, get_transformation_matrix, warp_affine
...
class V2XFusionBlock(nn.Module):
    def __init__(self, num_blocks, cav_att_config, pwindow_config):
        ...
        for _ in range(num_blocks):
            att = (
                HGTCavAttention(cav_att_config["dim"], heads=cav_att_config["heads"], dim_head=cav_att_config["dim_head"], dropout=cav_att_config["dropout"])
                if cav_att_config["use_hetero"]
                else CavAttention(cav_att_config["dim"], heads=cav_att_config["heads"], dim_head=cav_att_config["dim_head"], dropout=cav_att_config["dropout"])
            )
            self.layers.append(
                nn.ModuleList(
                    [
...
class V2XTEncoder(nn.Module):
    def __init__(self, args):
        ...
        for _ in range(depth):
            self.layers.append(
                nn.ModuleList(
                    [V2XFusionBlock(num_blocks, cav_att_config, pwindow_att_config), PreNorm(cav_att_config["dim"], FeedForward(cav_att_config["dim"], mlp_dim, dropout=dropout))]
                )
            )
