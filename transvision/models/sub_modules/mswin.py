...
from transvision.models.sub_modules.split_attn import SplitAttn
...
class BaseWindowAttention(nn.Module):
    ...
    def forward(self, x):
        _, _, h, w, _, m = *x.shape, self.heads
        ...
        # b l h w c
        out = rearrange(
            out, "b l m (new_h new_w) (w_h w_w) c -> b l (new_h w_h) (new_w w_w) (m c)", m=self.heads, w_h=self.window_size, w_w=self.window_size, new_w=new_w, new_h=new_h
        )
        out = self.to_out(out)
