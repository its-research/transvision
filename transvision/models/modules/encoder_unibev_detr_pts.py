...
class PtsEncoder(TransformerLayerSequence):
    """Attention with both self and cross Implements the encoder in BEVFormer
    transformer.
    ...
    def __init__(self, *args, pc_range=None, num_points_in_pillar_lidar=1, return_intermediate=False, dataset_type='nuscenes', **kwargs):
    ...
    def forward(self,
        ...
        ref_3d_lidar = self.get_reference_points(
            bev_h, bev_w, self.pc_range[5] - self.pc_range[2], self.num_points_in_pillar_lidar, dim='3d', bs=bev_query.size(1), device=bev_query.device, dtype=bev_query.dtype)
