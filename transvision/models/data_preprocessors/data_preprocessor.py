...
class Det3DDataDAIRPreprocessor(DetDataPreprocessor):
    ...
    def simple_process(self, data: dict, training: bool = False) -> dict:
        ...
        if 'infrastructure_points' in inputs:
            ...
            if self.voxel:
                voxel_dict = self.inf_voxelize(inputs['infrastructure_points'], data_samples)
        ...
        if 'infrastructure_t0_points' in inputs:
            ...
            if self.voxel:
                voxel_dict = self.inf_voxelize(inputs['infrastructure_t0_points'], data_samples)
        ...
        if 'infrastructure_t1_points' in inputs:
            ...
            if self.voxel:
                voxel_dict = self.inf_voxelize(inputs['infrastructure_t1_points'], data_samples)
        ...
        if 'infrastructure_t2_points' in inputs:
            ...
            if self.voxel:
                voxel_dict = self.inf_voxelize(inputs['infrastructure_t2_points'], data_samples)
    ...
    def preprocess_img(self, _batch_img: Tensor) -> Tensor:
        ...
        if self._enable_normalize:
            if self.mean.shape[0] == 3:
                assert _batch_img.dim() == 3 and _batch_img.shape[0] == 3, ('If the mean has 3 values, the input tensor '
    ...
    def _get_pad_shape(self, data: dict) -> List[Tuple[int, int]]:
        ...
        # Process data with `default_collate`.
            ...
            assert _batch_inputs.dim() == 4, ('The input of `ImgDataPreprocessor` should be a NCHW tensor '
    ...
    def voxelize(self, points: List[Tensor], data_samples: SampleList) -> Dict[str, Tensor]:
        ...
        if self.voxel_type == 'hard':
            ...
            for i, (res, data_sample) in enumerate(zip(points, data_samples)):
                ...
                res_coors = torch.floor((polar_res_clamp - min_bound) / polar_res_clamp.new_tensor(self.voxel_layer.voxel_size)).int()
    ...
    @torch.no_grad()
    def inf_voxelize(self, points: List[Tensor], data_samples: SampleList) -> Dict[str, Tensor]:
        """Apply voxelization to point cloud.
        ...
        if self.voxel_type == 'hard':
            ...
            for i, res in enumerate(points):
                ...
                res_voxel_centers = (res_coors[:, [2, 1, 0]] + 0.5) * res_voxels.new_tensor(self.inf_voxel_layer.voxel_size) + res_voxels.new_tensor(
                    self.inf_voxel_layer.point_cloud_range[0:3])
                res_coors = F.pad(res_coors, (1, 0), mode='constant', value=i)
                voxels.append(res_voxels)
                coors.append(res_coors)
                num_points.append(res_num_points)
            ...
            # dynamic voxelization only provide a coors mapping
            for i, res in enumerate(points):
                res_coors = self.inf_voxel_layer(res)
                res_coors = F.pad(res_coors, (1, 0), mode='constant', value=i)
                coors.append(res_coors)
            voxels = torch.cat(points, dim=0)
            coors = torch.cat(coors, dim=0)
            ...
            for i, (res, data_sample) in enumerate(zip(points, data_samples)):
                ...
                phi = torch.atan2(res[:, 1], res[:, 0])
                polar_res = torch.stack((rho, phi, res[:, 2]), dim=-1)
                min_bound = polar_res.new_tensor(self.inf_voxel_layer.point_cloud_range[:3])
                max_bound = polar_res.new_tensor(self.inf_voxel_layer.point_cloud_range[3:])
                try:  # only support PyTorch >= 1.9.0
                    polar_res_clamp = torch.clamp(polar_res, min_bound, max_bound)
                except TypeError:
                    polar_res_clamp = polar_res.clone()
                    for coor_idx in range(3):
                        polar_res_clamp[:, coor_idx][polar_res[:, coor_idx] > max_bound[coor_idx]] = max_bound[coor_idx]
                        polar_res_clamp[:, coor_idx][polar_res[:, coor_idx] < min_bound[coor_idx]] = min_bound[coor_idx]
                res_coors = torch.floor((polar_res_clamp - min_bound) / polar_res_clamp.new_tensor(self.inf_voxel_layer.voxel_size)).int()
                self.get_voxel_seg(res_coors, data_sample)
                res_coors = F.pad(res_coors, (1, 0), mode='constant', value=i)
                res_voxels = torch.cat((polar_res, res[:, :2], res[:, 3:]), dim=-1)
                voxels.append(res_voxels)
            ...
            voxels, coors = [], []
            voxel_size = points[0].new_tensor(self.inf_voxel_layer.voxel_size)
            for i, (res, data_sample) in enumerate(zip(points, data_samples)):
                res_coors = torch.round(res[:, :3] / voxel_size).int()
                res_coors -= res_coors.min(0)[0]
    ...
    def get_voxel_seg(self, res_coors: Tensor, data_sample: SampleList) -> None:
        ...
        if self.training:
            ...
            voxel_semantic_mask, _, point2voxel_map = dynamic_scatter_3d(F.one_hot(pts_semantic_mask.long()).float(), res_coors, 'mean', True)
