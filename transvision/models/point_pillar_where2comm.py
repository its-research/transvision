...
# from transvision.models.fuse_modules.where2comm import Where2comm
from transvision.models.fuse_modules.where2comm_attn import Where2comm
from transvision.models.sub_modules.base_bev_backbone import BaseBEVBackbone
from transvision.models.sub_modules.base_bev_backbone_resnet import ResNetBEVBackbone
from transvision.models.sub_modules.dcn_net import DCNNet
    ...
    def forward(self, data_dict):
        ...
        if self.multi_scale:
            fused_feature, communication_rates, result_dict = self.fusion_net(
                batch_dict["spatial_features"], psm_single, record_len, pairwise_t_matrix, self.backbone, [self.shrink_conv, self.cls_head, self.reg_head]
            )
            # downsample feature to reduce memory
            if self.shrink_flag:
                fused_feature = self.shrink_conv(fused_feature)
        else:
            fused_feature, communication_rates, result_dict = self.fusion_net(spatial_features_2d, psm_single, record_len, pairwise_t_matrix)
        ...
        output_dict.update({"psm_single_v": psm_single_v, "psm_single_i": psm_single_i, "rm_single_v": rm_single_v, "rm_single_i": rm_single_i, "comm_rate": communication_rates})
        return output_dict
