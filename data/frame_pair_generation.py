...
def idx_batch_mapping(inf_data_infos):
    ...
    for inf_data_info in inf_data_infos:
        inf_idx = inf_data_info['pointcloud_path'].split('/')[-1].replace(
            '.pcd', '')
...
def split_datas(data_infos, split_datas, split='val'):
    ...
    for data_info in data_infos:
        infrastructure_frame = data_info['infrastructure_image_path'].split(
            '/')[-1].replace('.jpg', '')
...
def data_info_flow_train(data_infos, inf_idx_batch_mappings):
    ...
    for data_info in data_infos:
        ...
        for ii in range(random_num):
            ...
            if t_1 not in infrastructure_idxs or inf_idx_batch_mappings[
                    infrastructure_idx] != inf_idx_batch_mappings[t_1]:
                continue
            data_info_flow['infrastructure_idx_t_1'] = t_1
            data_info_flow[
                'infrastructure_pointcloud_bin_path_t_1'] = 'infrastructure-side/velodyne/' + t_1 + '.bin'

            t_0 = str(int(t_1) - 1).zfill(6)
            if t_0 not in infrastructure_idxs or inf_idx_batch_mappings[
                    infrastructure_idx] != inf_idx_batch_mappings[t_0]:
                continue
            data_info_flow['infrastructure_idx_t_0'] = t_0
            data_info_flow[
                'infrastructure_pointcloud_bin_path_t_0'] = 'infrastructure-side/velodyne/' + t_0 + '.bin'
            data_info_flow['infrastructure_t_0_1'] = 1.0

            delta_t_2 = random.randint(1, 2)
            t_2 = str(int(t_1) + delta_t_2).zfill(6)
            if t_2 not in infrastructure_idxs or inf_idx_batch_mappings[
                    infrastructure_idx] != inf_idx_batch_mappings[t_2]:
                continue
            data_info_flow['infrastructure_idx_t_2'] = t_2
            data_info_flow[
                'infrastructure_pointcloud_bin_path_t_2'] = 'infrastructure-side/velodyne/' + t_2 + '.bin'
...
if __name__ == '__main__':
    ...
    inf_data_infos_path = os.path.join(dair_v2x_c_root,
    ...
    split_json_path = os.path.join('split_datas',
                                   'cooperative-split-data.json')
    split_jsons = read_json(split_json_path)

    # Generate training part
    data_infos_path = os.path.join(dair_v2x_c_root,
    ...
    data_infos_flow_train = data_info_flow_train(data_infos_train,
                                                 inf_idx_batch_mappings)
    ...
    for async_k in range(0, 6):
        data_infos_flow_val = data_info_flow_val(
            data_infos_val, inf_idx_batch_mappings, async_k=async_k)
        print('The length of data_infos_flow_val is: ', async_k,
              len(data_infos_flow_val))
        data_infos_flow_path = './data/dair-v2x/flow_data_jsons/flow_data_info_val_' + str(
            async_k) + '.json'
