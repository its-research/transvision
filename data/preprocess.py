...
def label_world2vlidar(sub_root, idx):
    path_input_label_file = os.path.join(sub_root, 'cooperative/label_world',
                                         idx + '.json')
    ...
    lidar_3d_list = []
    path_novatel2world = os.path.join(sub_root,
                                      'vehicle-side/calib/novatel_to_world',
                                      idx + '.json')
    path_lidar2novatel = os.path.join(sub_root,
                                      'vehicle-side/calib/lidar_to_novatel',
                                      idx + '.json')
    for label_world in input_label_data:
        world_8_points_old = label_world['world_8_points']
        world_8_points = []
        for point in world_8_points_old:
            point_new = trans_point_w2l(point, path_novatel2world,
                                        path_lidar2novatel)
        ...
        lidar_3d_data['3d_location']['x'] = (world_8_points[0][0] +
                                             world_8_points[2][0]) / 2
        lidar_3d_data['3d_location']['y'] = (world_8_points[0][1] +
                                             world_8_points[2][1]) / 2
        lidar_3d_data['3d_location']['z'] = (world_8_points[0][2] +
                                             world_8_points[4][2]) / 2
...
if __name__ == '__main__':
    ...
    for c_json in tqdm(c_jsons):
        ...
        calib_lidar_i2v_save_path = os.path.join(calib_lidar_i2v_save_dir,
                                                 veh_idx + '.json')
        ...
        pcd_path = os.path.join(
            dair_v2x_c_root,
            'infrastructure-side/velodyne/' + inf_idx + '.pcd')
        bin_save_path = os.path.join(
            dair_v2x_c_root,
            'infrastructure-side/velodyne/' + inf_idx + '.bin')
        pcd2bin(pcd_path, bin_save_path)
        c_json['infrastructure_pointcloud_bin_path'] = c_json[
            'infrastructure_pointcloud_path'].replace('.pcd', '.bin')
        c_json['infrastructure_idx'] = inf_idx

        pcd_path = os.path.join(dair_v2x_c_root,
                                'vehicle-side/velodyne/' + veh_idx + '.pcd')
        bin_save_path = os.path.join(
            dair_v2x_c_root, 'vehicle-side/velodyne/' + veh_idx + '.bin')
        pcd2bin(pcd_path, bin_save_path)
        c_json['vehicle_pointcloud_bin_path'] = c_json[
            'vehicle_pointcloud_path'].replace('.pcd', '.bin')
        ...
        c_json['cooperative_label_w2v_path'] = os.path.join(
            'cooperative/label/lidar', veh_idx + '.json')
        c_json['cooperative_label_path'] = os.path.join(
            'cooperative/label_world', veh_idx + '.json')

        c_json['image'] = {'image_shape': [1080, 1920]}

    c_jsons_write_path = os.path.join(dair_v2x_c_root,
    ...
    c_jsons_path = os.path.join(dair_v2x_c_root,
