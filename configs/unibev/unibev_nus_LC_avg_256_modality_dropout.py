...
test_pipeline = [
    ...
    dict(
        ...
        transforms=[
            dict(type='PadMultiViewImage', size_divisor=32),
            dict(type='DefaultFormatBundle3DBEVFusion', class_names=class_names, with_label=False),
...
model = dict(
    ...
    img_neck=dict(type='FPN', in_channels=[2048], out_channels=_dim_, start_level=0, add_extra_convs='on_output', num_outs=_num_levels_, relu_before_extra_convs=True),
    pts_bbox_head=dict(
        ...
        transformer=dict(
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        ...
                        dict(
                            ...
                            pc_range=point_cloud_range,
                            deformable_attention=dict(type='MSDeformableAttention3DUniQueryImg', embed_dims=_dim_, num_points=8, num_levels=_num_levels_),
                            embed_dims=_dim_,
                        )
                    ],
                    ffn_cfgs=dict(
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        ...
                        dict(
                            ...
                            pc_range=point_cloud_range,
                            deformable_attention=dict(type='MSDeformableAttention3DUniQueryPts', embed_dims=_dim_, num_points=8, num_levels=_num_levels_),
                            embed_dims=_dim_,
                        )
                    ],
                    ffn_cfgs=dict(
            ...
            decoder=dict(
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        dict(type='MultiheadAttention', embed_dims=_dim_ * dec_scale_factor, num_heads=8, dropout=0.1),
                    ...
                    ffn_dropout=0.1,
                    operation_order=('self_attn', 'norm', 'cross_attn', 'norm', 'ffn', 'norm')))),
        bbox_coder=dict(type='NMSFreeCoder', post_center_range=[-61.2, -61.2, -10.0, 61.2, 61.2, 10.0], pc_range=point_cloud_range, max_num=300, num_classes=10),
...
lr_config = dict(policy='CosineAnnealing', warmup='linear', warmup_iters=125, warmup_ratio=1.0 / 3, min_lr_ratio=1e-3)

# runtime settings
total_epochs = max_epochs

checkpoint_config = dict(interval=save_interval)
log_config = dict(interval=log_interval, hooks=[dict(type='TextLoggerHook'), dict(type='TensorboardLoggerHook'), dict(type='WandbLoggerHook')])
# yapf:enable
dist_params = dict(backend='nccl')
log_level = 'INFO'
custom_hooks = [dict(type='CheckpointLateStageHook', start=21, priority=60)]
