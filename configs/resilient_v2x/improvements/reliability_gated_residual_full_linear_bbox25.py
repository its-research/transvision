"""P7: P6 with only the student bbox loss raised to 2.5."""

_base_ = ["./reliability_gated_residual_full_linear.py"]

model = dict(bbox_head=dict(loss_bbox=dict(loss_weight=2.5)))
experiment = dict(
    _delete_=True,
    name="dair_improvement_reliability_gated_residual_full_linear_bbox25",
    stage="resilient_improvement",
    improvement=dict(
        component="student detection bbox regression loss",
        setting="P6 with student bbox loss 2.5; frozen teacher remains 2.0",
    ),
    candidate_provenance=dict(
        ...
        base_experiment="dair_improvement_reliability_gated_residual_full_linear",
