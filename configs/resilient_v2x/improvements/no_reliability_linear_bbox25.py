"""Preferred E2 follow-up with student bbox loss weight 2.5."""

_base_ = ["./no_reliability_linear.py"]

model = dict(bbox_head=dict(loss_bbox=dict(loss_weight=2.5)))

experiment = dict(
    _delete_=True,
    name="dair_improvement_no_reliability_linear_bbox25",
    stage="resilient_improvement",
    improvement=dict(
        component="linear PTF no-reliability student bbox regression loss",
        setting=(
            "E2 (linear PTF, no branch-reliability features) with student "
            "bbox loss weight 2.5; frozen teacher remains 2.0"
        ),
    ),
