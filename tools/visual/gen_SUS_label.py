...
def gen_sus_label_from_dair_v2x(root_path, output_path):
    """Generate SUS visualization file for DAIR-V2X label.(Take vehicle-side
    for example.)
    ...
    for file_name in list_input_file:
        ...
        os.system('cp %s/%s.jpg %s/camera/front/%s.jpg' % (veh_img_path, veh_frame_id, output_path, veh_frame_id))
        os.system('cp %s/%s.pcd %s/lidar/%s.pcd' % (veh_pt_path, veh_frame_id, output_path, veh_frame_id))
...
def gen_sus_from_kitti_file(side_flag, only_gen_label, dair_datasets_path, input_file_path, output_path):
    ...
    if side_flag == 'coop':
        names = [
            'frame', 'type', 'track_id', 'truncated', 'occlude', 'alpha', 'bbox-left', 'bbox-top', 'bbox-right', 'bbox-bottom', 'h', 'w', 'l', 'camera_bottom_center_x',
            'camera_bottom_center_y', 'camera_bottom_center_z', 'rotation_y', 'lidar_center_x', 'lidar_center_y', 'lidar_center_z', 'rotation_z', 'pointcloud_timestamp',
            'score_dtc', 'score_tracking', 'token', 'from_side', 'veh_pointcloud_timestamp', 'inf_pointcloud_timestamp', 'veh_frame_id', 'inf_frame_id', 'veh_track_id',
        ...
        names = [
            'frame', 'type', 'track_id', 'truncated', 'occlude', 'alpha', 'bbox-left', 'bbox-top', 'bbox-right', 'bbox-bottom', 'h', 'w', 'l', 'camera_bottom_center_x',
            'camera_bottom_center_y', 'camera_bottom_center_z', 'rotation_y', 'lidar_center_x', 'lidar_center_y', 'lidar_center_z', 'rotation_z', 'pointcloud_timestamp',
            'score_dtc', 'score_tracking', 'token'
        ]
    ...
    for frame in list(set(df['frame'])):
        ...
        if not only_gen_label:
            # lidar/camera
            os.system('cp %s/%s.jpg %s/%s.jpg' % (img_path, from_frame_id, output_path_camera_front, to_frame_id))
            os.system('cp %s/%s.pcd %s/%s.pcd' % (velodyne_path, to_frame_id, output_path_lidar, to_frame_id))
