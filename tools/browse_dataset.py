...
def main():
    ...
    try:
        dataset = DATASETS.build(cfg.train_dataloader.dataset, default_args=dict(filter_empty_gt=False))
    ...
    for i, item in enumerate(dataset):
        ...
        out_file = osp.join(args.output_dir, f'{i}.jpg') if args.output_dir is not None else None

        # o3d_save_path is valid when args.not_show is False
        o3d_save_path = osp.join(args.output_dir, f'pc_{i}.png') if (args.output_dir is not None and vis_task in ['lidar_det', 'lidar_seg', 'multi-modality_det']
