#!/usr/bin/env python3
"""Plan or execute the sealed P1 two-method, two-new-seed ClearML matrix.
...
is fixed to ResilientV2X and BEVFusion at the two pre-derived training seeds.
"""
...
EXECUTION_TOKEN = "ENQUEUE_P1_MULTISEED_4_TRAIN_4_EVAL"
PREFLIGHT_RECEIPT_TYPE = "resilient_v2x_p1_scoped_multiseed_preflight"
EXECUTION_KEY_TAG_PREFIX = "resilient-v2x-p1-execution:"
CONTROLLER_TAG = "resilient-v2x-p1-controller-lease"
ORPHAN_TAG = "resilient-v2x-p1-quarantined-orphan"
JOURNAL_ARTIFACT = "p1_multiseed_execution_journal"
MANIFEST_ARTIFACT = "p1_multiseed_execution_manifest"
PROJECT_ID = "6e43f972e5ea4cee901a7c8855fce8cd"
PROJECT_NAME = "ResilientV2X/Training"
FILES_SERVER_URI = "http://10.100.34.118:8081"
QUEUE_NAME = "GPU4-5090"
...
TRAINING_SEEDS = (2_856_275_757, 2_517_382_461)
...
def build_plan() -> dict[str, object]:
    """Return the one allowed P1 delta plan."""

    pairs = [
        _task_record(subject, seed_index, seed)
        for seed_index, seed in zip((2, 3), TRAINING_SEEDS, strict=True)
        for subject in SUBJECTS
    ]
    plan: dict[str, object] = {
        "schema_version": 1,
        "document_type": "resilient_v2x_p1_scoped_multiseed_plan",
        ...
        "scope": {
            ...
            "anchor_training_seed": TRAINING_OVERLAY_PROTOCOL_SEED,
            "new_training_task_count": 4,
            "new_evaluation_task_count": 4,
            "new_evaluation_condition_count": 48,
...
def validate_plan(value: Mapping[str, object]) -> None:
    """Fail closed if the serialized plan expands beyond 4 train + 4 eval."""
    ...
    _require_equal(
        ...
        "resilient_v2x_p1_scoped_multiseed_plan",
    ...
    if not isinstance(pairs, list) or len(pairs) != 4:
        raise P1ExecutorError("P1 plan must contain exactly four train/eval pairs")
    expected_pairs = [
        _task_record(subject, seed_index, seed)
        for seed_index, seed in zip((2, 3), TRAINING_SEEDS, strict=True)
        for subject in SUBJECTS
...
def build_pinset() -> dict[str, object]:
    ...
    pinset: dict[str, object] = {
        "schema_version": 1,
        "document_type": "resilient_v2x_p1_scoped_multiseed_pinset",
        "selector": {
...
def _controller_name(execution_key: str) -> str:
    return f"ResilientV2X P1 scoped 4-train 4-eval {execution_key} controller"


def _lease_queue_name(execution_key: str) -> str:
    return f"__p1_resilient_v2x_mutex_{_sha256(execution_key, 'execution key')}"
...
def _task_name(execution_key: str, task_key: str) -> str:
    ...
    if (
        re.fullmatch(r"(?:train|eval)-r01-s0[23]-(?:resilient_v2x|bevfusion)", task_key)
        is None
    ):
        raise P1ExecutorError("P1 task key is outside the fixed scope")
    return f"ResilientV2X P1 multiseed {execution_key} {task_key}"
    ...
    *,
    ...
    if task is None:
        task = clone(
            ...
            name=name,
            comment="Sealed P1 two-method/two-new-seed recoverable task.",
...
def _validate_worker_and_queue(task: object, context: str) -> None:
    ...
    if re.fullmatch(r"[^\s]+-5090:gpu(?:0,1,2,3|4,5,6,7)", worker) is None:
        raise P1ExecutorError(f"{context} did not run on a pinned 4xRTX5090 worker")
    queue = str(
        getattr(getattr(getattr(task, "data", None), "execution", None), "queue", "")
        or ""
    )
...
def _validate_training_result(
    ...
    *,
    ...
    return {"model_id": model_id, "checkpoint_sha256": checkpoint_sha}
...
def _validate_evaluation_result(
    ...
    *,
    ...
    _validate_metrics(
        metrics, subject=subject, checkpoint_sha256=model["checkpoint_sha256"]
    )
...
def _read_queue_snapshot(task_class: object) -> dict[str, object]:
    ...
    for raw_worker in worker_rows:
        ...
        if re.fullmatch(r"[^\s]+-5090:gpu(?:0,1,2,3|4,5,6,7)", worker_id) is None:
    ...
    if not attached:
        raise P1ExecutorError("target queue has no recently reporting pinned worker")
    return {
        "queue_id": QUEUE_ID,
        "queue_name": QUEUE_NAME,
...
def _validate_quarantined_preflight_controller(
    ...
    *,
    ...
    artifacts = _artifact_inventory(task)
    initialization = {
        "p1_multiseed_plan": plan,
        "p1_multiseed_pinset": pinset,
    }
...
def preflight(
    *,
    ...
    queue = _mapping(
        (queue_reader or _read_queue_snapshot)(Task), "preflight queue receipt"
    )
    required_queue = {
        "queue_id": QUEUE_ID,
        "queue_name": QUEUE_NAME,
    }
    for field, expected in required_queue.items():
        _require_equal(queue.get(field), expected, f"preflight queue {field}")
    workers = queue.get("workers")
    if (
        type(queue.get("registered_worker_count")) is not int
        or int(queue["registered_worker_count"]) < 1
        or not isinstance(workers, list)
        or len(workers) != queue["registered_worker_count"]
    ):
        raise P1ExecutorError("preflight queue has no exact worker receipt")
    lease = _mapping(
...
def _quarantine_new_controller_if_pristine(
    ...
    *,
    ...
    artifacts = _artifact_inventory(controller)
    initialization_artifacts = {"p1_multiseed_plan", "p1_multiseed_pinset"}
...
def _find_or_create_controller(
    ...
    *,
    ...
    if controller is None:
        ...
        controller = _fresh(task_class, controller_id)
        _upload_artifact(controller, "p1_multiseed_plan", plan)
        _upload_artifact(controller, "p1_multiseed_pinset", pinset)
        controller = _select_unique_created_candidate(
            task_class, name=name, context="P1 execution controller"
        )
        if controller is None:
...
def _new_journal(
    *, plan: Mapping[str, object], execution_key: str, controller_id: str
    ...
    journal: dict[str, object] = {
        "schema_version": 1,
        "document_type": "resilient_v2x_p1_scoped_multiseed_journal",
        "execution_key": execution_key,
        "plan_seal_sha256": plan["seal_sha256"],
...
def _validate_journal(
    ...
    *,
    ...
    if not isinstance(observed, list) or len(observed) != 8:
        raise P1ExecutorError("P1 journal must contain exactly eight task slots")
...
def _mutex_receipt_path() -> Path:
    return _validate_cache_directory() / ".p1-scoped-multiseed-mutex.json"
...
def _server_execution_mutex(
    ...
    *,
    ...
    if recovered is not None:
        ...
        tags = ["p1-scoped-mutex", _execution_tag(execution_key), owner_tag]
        try:
...
def _validate_execution_manifest(
    ...
    *,
    ...
    if not isinstance(results, list) or len(results) != 4:
        raise P1ExecutorError("P1 execution manifest must contain four results")
    expected_pairs = [
    ...
    for result, record in zip(results, expected_pairs, strict=True):
        ...
        _clearml_id(result.get("training_task_id"), "P1 manifest training task")
        ...
        _clearml_id(evaluation.get("task_id"), "P1 manifest evaluation task")
    return manifest
    ...
    with _execution_lock():
        ...
        with _server_execution_mutex(execution_key, api_client=client):
            ...
            try:
                ...
                if len(manifest["results"]) != 4:
                    raise P1ExecutorError(
                        "executor did not complete exactly four pairs"
                    )
