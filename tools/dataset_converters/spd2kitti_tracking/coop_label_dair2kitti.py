...
from tools.dataset_converter.utils import get_camera_3d_alpha_rotation, get_lidar2camera, get_lidar_3d_8points, read_json, trans_point
...
parser.add_argument('--target-root', type=str, default='data/KITTI-Track/cooperative', help='The data root where the data with kitti-Track format is generated')
parser.add_argument('--split-path', type=str, default='data/split_datas/cooperative-split-data-spd.json', help='Json file to split the data into training/validation/testing.')
...
def label_dair2kiiti_by_frame(dair_label_file_path, kitti_label_file_path, rotation, translation, frame, pointcloud_timestamp, no_classmerge):
    ...
    for label in list_labels:
        ...
        label_3d_dimensions = [float(label['3d_dimensions']['l']), float(label['3d_dimensions']['w']), float(label['3d_dimensions']['h'])]
        lidar_3d_location = [float(label['3d_location']['x']), float(label['3d_location']['y']), float(label['3d_location']['z'])]
        rotation_z = float(label['rotation'])
        lidar_3d_8_points = get_lidar_3d_8points(label_3d_dimensions, lidar_3d_location, rotation_z)

        lidar_3d_bottom_location = [float(label['3d_location']['x']), float(label['3d_location']['y']), float(label['3d_location']['z']) - float(label['3d_dimensions']['h']) / 2]
        ...
        list_item = [
            ...
            str(rotation_z), pointcloud_timestamp, '1', '1',
            str(label['token'])
        ]
