...
def get_semantickitti_info(split):
    ...
    for i_folder in fold_split[split]:
        for j in range(0, total_num[i_folder]):
            data_list.append(
                {
                    "lidar_points": {"lidar_path": osp.join("sequences", str(i_folder).zfill(2), "velodyne", str(j).zfill(6) + ".bin"), "num_pts_feats": 4},
                    "pts_semantic_mask_path": osp.join("sequences", str(i_folder).zfill(2), "labels", str(j).zfill(6) + ".label"),
