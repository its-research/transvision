...
from utils import get_cam_calib_intrinsic, get_camera_3d_alpha_rotation, get_label_lidar_rotation, get_lidar2camera, read_json, trans_point
...
def label_det_result2kitti(input_file_path, output_dir_path, spd_path):
    """Convert detection results from mmdetection3d_kitti format to KITTI
    format.
    ...
    for i in range(num_boxes):
        ...
        lidar_3d_8points = [
            lidar_3d_8points_det_result[3], lidar_3d_8points_det_result[0], lidar_3d_8points_det_result[4], lidar_3d_8points_det_result[7], lidar_3d_8points_det_result[2],
            lidar_3d_8points_det_result[1], lidar_3d_8points_det_result[5], lidar_3d_8points_det_result[6]
        ]
        ...
        str_item = str(veh_frame) + ' ' + str(obj_type) + ' ' + '-1' + ' ' + '-1' + ' ' + '-1' + ' ' + str(alpha) + ' ' + str(x_min) + ' ' + str(y_min) + ' ' + str(
            x_max) + ' ' + str(y_max) + ' ' + str(h) + ' ' + str(w) + ' ' + str(l) + ' ' + str(camera_x) + ' ' + str(camera_y) + ' ' + str(camera_z) + ' ' + str(
                camera_rotation) + ' ' + str(lidar_x) + ' ' + str(lidar_y) + ' ' + str(lidar_z) + ' ' + str(lidar_rotation) + ' ' + '-1' + ' ' + str(
                    score) + ' ' + '-1' + ' ' + '-1' + '\n'
...
def gen_kitti_result(input_dir_path, output_dir_path, spd_path):
    """Convert detection results from mmdetection3d_kitti format to KITTI
    format for all files in input_dir_path.
...
def gen_kitti_seq_result(input_dir_path, output_dir_path, spd_path):
    """Convert detection results from mmdetection3d_kitti format to KITTI
    format and group them by sequence.
...
def gen_kitti_seq_txt(input_dir_path, output_dir_path):
    """Group converted KITTI format files by sequence and write them into one
