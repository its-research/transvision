...
from tools.dataset_converter.utils import (get_cam_calib_intrinsic, get_camera_3d_alpha_rotation, get_label_lidar_rotation, get_lidar2camera, get_lidar2novatel,
                                           get_lidar_3d_8points, get_novatel2world, get_virtuallidar2world, inverse_matrix, read_json, trans_point, write_json)
...
def trans_label_i2v(dair_inf_label_path, dair_target_label_file, kitti_target_label_file, virtuallidar2world_path, novatel2world_path, lidar2novatel_path, lidar2camera_path,
                    camera2image_path, delta_x, delta_y, pointcloud_timestamp):
    ...
    for m in range(len(dair_inf_label_data)):
        # 提取每个目标的信息
        i_label_3d_dimensions = [
            float(dair_inf_label_data[m]['3d_dimensions']['l']),
            float(dair_inf_label_data[m]['3d_dimensions']['w']),
            float(dair_inf_label_data[m]['3d_dimensions']['h'])
        ]
        i_label_lidar_3d_location = [
            float(dair_inf_label_data[m]['3d_location']['x']),
        ...
        v_label_lidar_3d_location = trans_point_i2v(i_label_lidar_3d_location, virtuallidar2world_path, novatel2world_path, lidar2novatel_path, delta_x, delta_y)
        ...
        for i_lidar_point in list_i_lidar_3d_8_points:
            # 投影到车端lidar坐标系
            v_lidar_point = trans_point_i2v(i_lidar_point, virtuallidar2world_path, novatel2world_path, lidar2novatel_path, delta_x, delta_y)
        ...
        # 生成 kitti label
        list_kitti_label = [
            frame, dair_inf_label_data[m]['type'],
            str(dair_inf_label_data[m]['track_id']),
            str(dair_inf_label_data[m]['truncated_state']),
            str(dair_inf_label_data[m]['occluded_state']),
            str(dair_inf_label_data[m]['alpha']),
            str(dair_inf_label_data[m]['2d_box']['xmin']),
            str(dair_inf_label_data[m]['2d_box']['ymin']),
            str(dair_inf_label_data[m]['2d_box']['xmax']),
            str(dair_inf_label_data[m]['2d_box']['ymax']),
            str(dair_inf_label_data[m]['3d_dimensions']['h']),
            str(dair_inf_label_data[m]['3d_dimensions']['w']),
            str(dair_inf_label_data[m]['3d_dimensions']['l']),
            str(v_label_camera_3d_location[0]),
            str(v_label_camera_3d_location[1]),
            str(v_label_camera_3d_location[2]),
            str(i_label_camera_rotation),
            str(dair_inf_label_data[m]['3d_location']['x']),
            str(dair_inf_label_data[m]['3d_location']['y']),
            str(dair_inf_label_data[m]['3d_location']['z']),
...
def get_i2v(dair_source_path, dair_target_path, kitti_target_path, split_info, label_type):
    ...
    for i in track(coop_data_info):
        ...
        trans_label_i2v(dair_inf_label_path, dair_target_label_file, kitti_target_label_file, virtuallidar2world_path, novatel2world_path, lidar2novatel_path, lidar2camera_path,
                        camera2image_path, delta_x, delta_y, pointcloud_timestamp)


if __name__ == '__main__':
    parser = argparse.ArgumentParser('Convert The Label from Infrastructure-side to Vehicle-side')
    parser.add_argument('--dair-source-root', type=str, default='../../data/SPD/cooperative-vehicle-infrastructure', help='Raw data root about DAIR-V2X-C')
    parser.add_argument(
        '--dair-target-root',
        type=str,
    ...
    parser.add_argument(
        '--split-path', type=str, default='../../data/split_datas/cooperative-split-data-spd.json', help='Json file to split the data into training/validation/testing.')
