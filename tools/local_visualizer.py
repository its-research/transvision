...
from mmdet3d.structures import (BaseInstance3DBoxes, Box3DMode, CameraInstance3DBoxes, Coord3DMode, DepthInstance3DBoxes, DepthPoints, Det3DDataSample, LiDARInstance3DBoxes,
                                PointData, points_cam2img)
from mmdet3d.visualization.vis_utils import proj_camera_bbox3d_to_img, proj_depth_bbox3d_to_img, proj_lidar_bbox3d_to_img, to_depth_mode
...
class Det3DLocalVisualizer2(DetLocalVisualizer):
    ...
    def __init__(self,
                 ...
                 fig_show_cfg: dict = dict(figsize=(18, 12))) -> None:
        super().__init__(
            name=name,
            image=image,
    ...
    def draw_bboxes_3d(self,
        ...
        if isinstance(bbox_colors, list):
            assert len(bbox_colors) == len(bboxes_3d), 'specify color for every bounding box'
    ...
    def draw_bev_bboxes(self,
        ...
        return self.draw_polygons(poly, alpha=alpha, edge_colors=edge_colors, line_styles=line_styles, line_widths=line_widths, face_colors=face_colors)

    @master_only
    def draw_points_on_image(self, points: Union[np.ndarray, Tensor], pts2img: np.ndarray, sizes: Union[np.ndarray, int] = 3, max_depth: Optional[float] = None) -> None:
        ...
        self.ax_save.scatter(projected_points[:, 0], projected_points[:, 1], c=colors, cmap=color_map, s=sizes, alpha=0.7, edgecolors='none')
    ...
    def draw_proj_bboxes_3d(self,
        ...
        p = PatchCollection(pathpatches, facecolors='none', edgecolors=edge_colors_norm, linewidths=line_widths, linestyles=line_styles)
        ...
        return self.draw_polygons(front_polys, alpha=alpha, edge_colors=edge_colors, line_styles=line_styles, line_widths=line_widths, face_colors=edge_colors)
    ...
    def _draw_instances_3d(self,
        ...
        if vis_task in ['lidar_det', 'multi-modality_det']:
            ...
            self.set_points(points, pcd_mode=2, mode='xyzrgb' if show_pcd_rgb else 'xyz')
        ...
        if vis_task in ['mono_det', 'multi-modality_det']:
            ...
            if isinstance(img, list) or (isinstance(img, (np.ndarray, Tensor)) and len(img.shape) == 4):
                ...
                for i, single_img in enumerate(img):
                    ...
                    self.draw_proj_bboxes_3d(bboxes_3d, single_img_meta, img_size=single_img.shape[:2][::-1], edge_colors=colors)
                    if vis_task == 'mono_det' and hasattr(instances, 'centers_2d'):
                        centers_2d = instances.centers_2d
                        self.draw_points(centers_2d)
                    composed_img[(i // img_col) * img_size[0]:(i // img_col + 1) * img_size[0], (i % img_col) * img_size[1]:(i % img_col + 1) * img_size[1]] = self.get_image()
    ...
    def _draw_pts_sem_seg(self, points: Union[Tensor, np.ndarray], pts_seg: PointData, palette: Optional[List[tuple]] = None, keep_index: Optional[int] = None) -> None:
    ...
    def show(self,
        ...
        if hasattr(self, 'o3d_vis'):
            ...
            if wait_time != -1:
                ...
                while time.time() - self.last_time < wait_time and self.o3d_vis.poll_events():
                    self.o3d_vis.update_renderer()
                    self.view_port = \
                        self.view_control.convert_to_pinhole_camera_parameters()  # noqa: E501
    ...
    def space_action_callback(self, vis, action, mods):
        if action == 1:
            if self.flag_pause:
                print_log('Playback continued, press [SPACE] to pause.', logger='current')
            else:
                print_log('Playback paused, press [SPACE] to continue.', logger='current')
    ...
    def add_datasample(self,
        ...
        if draw_gt and data_sample is not None:
            if 'gt_instances_3d' in data_sample:
                gt_data_3d = self._draw_instances_3d(data_input, data_sample.gt_instances_3d, data_sample.metainfo, vis_task, show_pcd_rgb, palette)
        ...
        if draw_pred and data_sample is not None:
            if 'pred_instances_3d' in data_sample:
                ...
                pred_data_3d = self._draw_instances_3d(data_input, pred_instances_3d, data_sample.metainfo, vis_task, show_pcd_rgb, palette)
