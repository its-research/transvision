...
本节完整列出论文实验章节要求的结果表。当前仓库**没有任何满足提交条件的受控实测结果**；修复后的训练仍在进行，尚未完成“至少 3 个随机种子、同一协议、可验证证据包”的统计闭环。因此，下表保留论文结果登记表中的证据状态，不把旧稿数字、公开论文数字或正在训练的中间日志当作复现结果。结果登记表当前包含 113 个直接结果和 15 个派生结果，`verified=0`。
...
主指标：BEV AP@0.7。受控 L-Fail/C-Fail 均使用 `E+R`。表中的旧稿锚点只为保留记录，不能填充或验证相邻的受控待测单元格。

| 条件 | 0 ms | 100 ms | 200 ms | 300 ms |
| :--- | ---: | ---: | ---: | ---: |
| Full | 58.1† | —（待测） | 57.7† | 57.3† |
| L-Fail（† 的 scope 未知；受控定义为 E+R） | 28.6† | —（待测） | —（待测） | —（待测） |
| C-Fail（† 的 scope 未知；受控定义为 E+R） | 54.8† | —（待测） | —（待测） | —（待测） |
...
| :--- | :--- | ---: | ---: | ---: |
| Full nonlinear PTF + DER | 容量匹配的受控参考配置 | —（待测） | —（待测） | —（待测） |
...
| :--- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | :--- |
| DAIR-V2X-C / Resilient V2X / Full / 0 ms | —（待测） | —（待测） | —（待测） | —（待测） | —（待测） | —（待测） | —（待测） | —（待测） | 0 / ≥3 | 尚无 verified 证据包 |
