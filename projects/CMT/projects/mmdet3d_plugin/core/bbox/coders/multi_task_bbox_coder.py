    ...
    def decode(self, preds_dicts):
        ...
        for task_id in range(task_num):
            ...
            task_pred_bbox = torch.cat(
                (task_pred_dict['center'][-1], task_pred_dict['height'][-1], task_pred_dict['dim'][-1], task_pred_dict['rot'][-1], task_pred_dict['vel'][-1]), dim=-1)
        ...
        for i in range(batch_size):
            predictions_list.append(self.decode_single(all_pred_logits[i], all_pred_bbox[i], all_task_ids[i]))
