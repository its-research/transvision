...
dvr = load('dvr', sources=['projects/mmdet3d_plugin/ray_iou/lib/dvr/dvr.cpp', 'projects/mmdet3d_plugin/ray_iou/lib/dvr/dvr.cu'], verbose=True, extra_cuda_cflags=['-allow-unsupported-compiler'])
...
def process_one_sample(sem_pred, lidar_rays, output_origin, flow_pred):
    ...
    for t in range(T):
        ...
        with torch.no_grad():
            pred_dist, _, coord_index = dvr.render_forward(occ_pred.cuda(), output_origin_render.cuda(), output_points_render.cuda(), output_tindex_render.cuda(),
                                                           [1, 16, 200, 200], 'test')
...
def main(sem_pred_list, sem_gt_list, flow_pred_list, flow_gt_list, lidar_origin_list):
    ...
    for i in range(len(occ_class_names) - 1):
        table.add_row([occ_class_names[i], iou_list[0][i], iou_list[1][i], iou_list[2][i], ave_list[i]], divider=(i == len(occ_class_names) - 2))

    table.add_row(['MEAN', np.nanmean(iou_list[0]), np.nanmean(iou_list[1]), np.nanmean(iou_list[2]), np.nanmean(ave_list)])
