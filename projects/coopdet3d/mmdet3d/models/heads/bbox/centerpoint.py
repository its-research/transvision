...
class CenterHead(BaseModule):
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d):
        ...
        for idx, task_head in enumerate(self.task_heads):
            heatmap = gt_bboxes_3d.new_zeros((len(self.class_names[idx]), int(feature_map_size[1].item()), int(feature_map_size[0].item())))
    ...
    def get_bboxes(self, preds_dicts, metas, img=None, rescale=False):
        ...
        if 'nms_scale' in self.test_cfg:
            if not isinstance(self.test_cfg['nms_scale'], list):
                nms_scales = [[self.test_cfg['nms_scale'] for _ in range(self.num_classes[task_id])] for task_id in range(len(preds_dicts))]
            else:
                nms_scales = self.test_cfg['nms_scale']
        else:
            nms_scales = [[1.0 for _ in range(self.num_classes[task_id])] for task_id in range(len(preds_dicts))]
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if nms_types[task_id] == 'circle':
                ...
                rets.append(self.get_task_detections(
                    num_class_with_bg,
                    batch_cls_preds,
                    batch_reg_preds,
                    batch_cls_labels,
