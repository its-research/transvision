...
from mmdet3d.models.builder import build_backbone, build_coop_fuser, build_fusion_model_headless, build_head, build_neck
...
class BEVFusionCoop(Base3DCoopFusionModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if vehicle.get('fusion_model') is not None:
            self.nodes['vehicle'] = nn.ModuleDict({'fusion_model': build_fusion_model_headless(vehicle['fusion_model'])})
        else:
            self.nodes['vehicle'] = None
        if infrastructure.get('fusion_model') is not None:
            self.nodes['infrastructure'] = nn.ModuleDict({'fusion_model': build_fusion_model_headless(infrastructure['fusion_model'])})
        else:
    ...
    def forward_single(
        ...
        **kwargs,
        ...
        features = []
        batch_size = 0
        for node in (self.nodes if self.training else list(self.nodes.keys())[::-1]):
            if node == 'vehicle' and self.nodes[node] is not None:
                feature, bs = self.nodes[node]['fusion_model'].forward(vehicle_img, vehicle_points, vehicle_lidar2camera, vehicle_lidar2image, vehicle_camera_intrinsics,
                                                                       vehicle_camera2lidar, vehicle_img_aug_matrix, vehicle_lidar_aug_matrix, vehicle2infrastructure, node, metas)
            elif node == 'vehicle' and self.nodes[node] is None:
                continue
            elif node == 'infrastructure' and self.nodes[node] is not None:
                feature, bs = self.nodes[node]['fusion_model'].forward(infrastructure_img, infrastructure_points, infrastructure_lidar2camera, infrastructure_lidar2image,
                                                                       infrastructure_camera_intrinsics, infrastructure_camera2lidar, infrastructure_img_aug_matrix,
                                                                       infrastructure_lidar_aug_matrix, vehicle2infrastructure, node, metas)
            elif node == 'infrastructure' and self.nodes[node] is None:
                continue
            else:
