...
class ImageAug3DCoop:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for img_vehicle in imgs_vehicle:
            resize_vehicle, resize_dims_vehicle, crop_vehicle, flip_vehicle, rotate_vehicle = self.sample_augmentation(data, True)
        ...
        for img_infrastructure in imgs_infrastructure:
            resize_infrastructure, resize_dims_infrastructure, crop_infrastructure, flip_infrastructure, rotate_infrastructure = self.sample_augmentation(data, False)
...
class GridMaskCoop:
    ...
    def __call__(self, results):
        ...
        mask1 = mask1[(hh1 - h1) // 2:(hh1 - h1) // 2 + h1, (ww1 - w1) // 2:(ww1 - w1) // 2 + w1]
        mask2 = Image.fromarray(np.uint8(mask2))
        mask2 = mask2.rotate(r2)
        mask2 = np.asarray(mask2)
        mask2 = mask2[(hh2 - h2) // 2:(hh2 - h2) // 2 + h2, (ww2 - w2) // 2:(ww2 - w2) // 2 + w2]
...
class RandomFlip3DCoop:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        data['infrastructure_lidar_aug_matrix'][:3, :] = rotation_infrastructure @ data['infrastructure_lidar_aug_matrix'][:3, :]
        return data
...
class PointSample:
    ...
    def __call__(self, data):
        ...
        if self.sample_range is not None:
            ...
            assert isinstance(points, CameraPoints), 'Sampling based on distance is only applicable for CAMERA coord'
...
class BackgroundPointsFilter:
    ...
    def __init__(self, bbox_enlarge_range):
        assert (is_tuple_of(bbox_enlarge_range, float) and len(bbox_enlarge_range) == 3) or isinstance(bbox_enlarge_range,
                                                                                                       float), f'Invalid arguments bbox_enlarge_range {bbox_enlarge_range}'
...
class ImageDistort:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        new_imgs = []
        for img in imgs:
            assert img.dtype == np.float32, ('PhotoMetricDistortion needs the input image of dtype np.float32,'
