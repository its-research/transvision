...
def main():
    ...
    for key in model:
        if key.startswith(
                'nodes.vehicle.fusion_model.encoders.lidar.backbone') or key.startswith('nodes.infrastructure.fusion_model.encoders.lidar.backbone') and '.bn.' not in key:
        ...
        if is_sparseconv_weight:
            ...
            if converted_weights.shape[0] == 1:
                ...
                kykx = ky * kx
                offsets = [(x * kykx + y * kx + z) for z in offsets[0] for y in offsets[1] for x in offsets[2]]
                offsets = torch.tensor(offsets, dtype=torch.int64, device=converted_weights.device)
