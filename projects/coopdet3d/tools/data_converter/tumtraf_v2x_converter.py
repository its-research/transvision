...
class TUMTrafV2X2NuScenes(object):
    ...
    This class serves as the converter to change the TUMTraf-V2X data to nuScenes format format.
    """

    def __init__(self, splits, load_dir, save_dir, name_format='name'):
        """
        Args:
        ...
        self.vehicle_point_cloud_save_dir = 'vehicle_lidar_robosense'
        self.infrastructure_point_cloud_save_dir = 's110_lidar_ouster_south'
        self.registered_point_cloud_save_dir = 's110_lidar_ouster_south_and_vehicle_lidar_robosense_registered'
        ...
        self.imagesets = {'training': self.train_set, 'validation': self.val_set, 'testing': self.test_set}
    ...
    def convert(self):
        ...
        for split in self.splits:
            ...
            vehicle_input_path = os.path.join(self.load_dir, self.map_version_to_dir[split], 'point_clouds', 'vehicle_lidar_robosense', '*')
            print('vehicle_input_path: ', vehicle_input_path)
            vehicle_pcd_file_paths_sorted = sorted(glob(vehicle_input_path))
            print('vehicle_pcd_file_paths_sorted: ', vehicle_pcd_file_paths_sorted)
            infrastructure_pcd_file_paths_sorted = sorted(glob(os.path.join(self.load_dir, self.map_version_to_dir[split], 'point_clouds', 's110_lidar_ouster_south', '*')))
            print('infrastructure_pcd_file_paths_sorted: ', infrastructure_pcd_file_paths_sorted)
            registered_pcd_file_paths_sorted = sorted(
                glob(os.path.join(self.load_dir, self.map_version_to_dir[split], 'point_clouds', 's110_lidar_ouster_south_and_vehicle_lidar_robosense_registered', '*')))
            ...
            for idx, vehicle_pcd_file_path in enumerate(vehicle_pcd_file_paths_sorted):
                ...
                self.save_lidar(vehicle_pcd_file_path, os.path.join(self.vehicle_point_cloud_save_dir, out_filename_no_ext))
                vehicle_pcd_file_paths_sorted[idx] = os.path.join(self.vehicle_point_cloud_save_dir, out_filename_no_ext) + '.bin'
            ...
            for idx, infrastructure_pcd_file_path in enumerate(infrastructure_pcd_file_paths_sorted):
                ...
                self.save_lidar(infrastructure_pcd_file_path, os.path.join(self.infrastructure_point_cloud_save_dir, out_filename_no_ext))
                infrastructure_pcd_file_paths_sorted[idx] = os.path.join(self.infrastructure_point_cloud_save_dir, out_filename_no_ext) + '.bin'
            ...
            for idz, registered_pcd_file_path in enumerate(registered_pcd_file_paths_sorted):
                ...
                self.save_lidar(registered_pcd_file_path, os.path.join(self.registered_point_cloud_save_dir, out_filename_no_ext))
                registered_pcd_file_paths_sorted[idz] = os.path.join(self.registered_point_cloud_save_dir, out_filename_no_ext) + '.bin'
            ...
            img_north_list = sorted(glob(os.path.join(self.load_dir, self.map_version_to_dir[split], 'images', 's110_camera_basler_north_8mm', '*')))
            img_vehicle_list = sorted(glob(os.path.join(self.load_dir, self.map_version_to_dir[split], 'images', 'vehicle_camera_basler_16mm', '*')))
            # TODO: do not run this for test (because no labels). Use empty list instead
            pcd_labels_list = sorted(
                glob(os.path.join(self.load_dir, self.map_version_to_dir[split], 'labels_point_clouds', 's110_lidar_ouster_south_and_vehicle_lidar_robosense_registered', '*')))

            infos_list = self._fill_infos(vehicle_pcd_file_paths_sorted, infrastructure_pcd_file_paths_sorted, registered_pcd_file_paths_sorted, img_south1_list, img_south2_list,
                                          img_north_list, img_vehicle_list, pcd_labels_list, test)
    ...
    def _fill_infos(self,
                    vehicle_pcd_list,
                    infrastructure_pcd_list,
                    registered_pcd_list,
                    img_south1_list,
                    img_south2_list,
                    img_north_list,
        ...
        infralidar2s1image = np.asarray(
            [[1279.275240545117, -862.9254609474538, -443.6558546306608, -16164.33175985643], [-57.00793327192514, -67.92432779187584, -1461.785310749125, -806.9258947569469],
             [0.7901272773742676, 0.3428181111812592, -0.508108913898468, 3.678680419921875]],
            dtype=np.float32)

        infralidar2s2image = np.asarray([[1546.63215008, -436.92407115, -295.58362676, 1319.79271737], [93.20805656, 47.90351592, -1482.13403199, 687.84781276],
                                         [0.73326062, 0.59708904, -0.32528854, -1.30114325]],
                                        dtype=np.float32)

        infralidar2n1image = np.asarray(
            [[-185.2891049687059, -1504.063395597006, -525.9215327879701, -23336.12843138125], [-240.2665682659353, 220.6722195428702, -1567.287260600104, 6362.243306159624],
             [0.6863989233970642, -0.4493367969989777, -0.5717979669570923, -6.750176429748535]],
            dtype=np.float32)

        south1intrinsics = np.asarray([[1400.3096617691212, 0.0, 967.7899705163408], [0.0, 1403.041082755918, 581.7195041357244], [0.0, 0.0, 1.0]], dtype=np.float32)

        # transformation matrix 4x4
        south12infralidar = np.asarray([[0.41204962, -0.45377758, 0.7901276, 2.158825], [-0.9107832, -0.23010845, 0.34281868, -15.5765505],
                                        [0.02625162, -0.86089253, -0.5081085, 0.08758777], [0.0, 0.0, 0.0, 1.0]],
                                       dtype=np.float32)

        # extrinsic matrix 3x4
        south12infralidar = south12infralidar[:-1, :]

        south2intrinsics = np.asarray([[1029.2795655594014, 0.0, 982.0311857478633], [0.0, 1122.2781391971948, 1129.1480997238505], [0.0, 0.0, 1.0]], dtype=np.float32)

        south22infralidar = np.asarray([[0.6353517, -0.24219051, 0.7332613, -0.03734626], [-0.7720766, -0.217673, 0.5970893, 2.5209506],
                                        [0.01500183, -0.9454958, -0.32528937, 0.543223], [0.0, 0.0, 0.0, 1.0]],
                                       dtype=np.float32)

        south22infralidar = south22infralidar[:-1, :]

        northintrinsics = np.asarray([[1315.158203125, 0.0, 962.7348338975571], [0.0, 1362.7757568359375, 580.6482296623581], [0.0, 0.0, 1.0]], dtype=np.float32)

        north2infralidar = np.asarray([[-0.56460226, -0.4583457, 0.6863989, 0.64204305], [-0.8248329, 0.34314296, -0.4493365, -16.182753],
                                       [-0.02958117, -0.81986094, -0.57179797, 1.6824605], [0.0, 0.0, 0.0, 1.0]],
                                      dtype=np.float32)
        ...
        vehiclelidar2image = np.asarray(
            [[1019.929965441548, -2613.286262078907, 184.6794570200418, 370.7180273597151], [589.8963703919744, -24.09642935106967, -2623.908527352794, -139.3143336725661],
             [0.9841844439506531, 0.1303769648075104, 0.1199281811714172, -0.1664766669273376]],
            dtype=np.float32)

        vehiclecamintrinsics = np.asarray([[2726.55, 0.0, 685.235], [0.0, 2676.64, 262.745], [0.0, 0.0, 1.0]], dtype=np.float32)

        vehiclecam2lidar = np.asarray(
            [
                [0.12672871, 0.12377692, 0.9841849, 0.14573078],  # TBD
        ...
        for i, pcd_path in enumerate(infrastructure_pcd_list):
            ...
            info = {
                ...
                lidar_anno_frame['frame_properties']['transforms']['vehicle_lidar_robosense_to_s110_lidar_ouster_south']['transform_src_to_dst']['matrix4x4'],
    ...
    def create_folder(self, split):
        ...
        split_path = self.map_version_to_dir[split]
        # print(split_path)
        self.infrastructure_point_cloud_save_dir = os.path.join(self.save_dir, split_path, 'point_clouds/s110_lidar_ouster_south')
        ...
        self.vehicle_point_cloud_save_dir = os.path.join(self.save_dir, split_path, 'point_clouds/vehicle_lidar_robosense')
        # print(self.vehicle_point_cloud_save_dir)
        os.makedirs(self.vehicle_point_cloud_save_dir, exist_ok=True, mode=0o777)
        self.registered_point_cloud_save_dir = os.path.join(self.save_dir, split_path, 'point_clouds/s110_lidar_ouster_south_and_vehicle_lidar_robosense_registered')
