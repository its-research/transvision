...
class TUMTraf2NuScenes(object):
    ...
    This class serves as the converter to change the TUMTraf data to nuScenes format.
    """
    ...
    def _fill_infos(self, pcd_list, img_south1_list, img_south2_list, pcd_labels_list, img_south1_labels_list, img_south2_labels_list, test=False):
        ...
        lidar2ego = np.asarray([[0.99011437, -0.13753536, -0.02752358, 2.3728100375737995], [0.13828977, 0.99000475, 0.02768645, -16.19297517556697],
                                [0.02344061, -0.03121898, 0.99923766, -8.620000000000005], [0.0, 0.0, 0.0, 1.0]],
                               dtype=np.float32)

        lidar2ego = lidar2ego[:-1, :]

        lidar2s1image = np.asarray([[7.04216073e02, -1.37317442e03, -4.32235765e02, -2.03369364e04], [-9.28351327e01, -1.77543929e01, -1.45629177e03, 9.80290034e02],
                                    [8.71736000e-01, -9.03453000e-02, -4.81574000e-01, -2.58546000e00]],
                                   dtype=np.float32)

        lidar2s2image = np.asarray([[1546.63215008, -436.92407115, -295.58362676, 1319.79271737], [93.20805656, 47.90351592, -1482.13403199, 687.84781276],
        ...
        south12ego = np.asarray([[-0.06377762, -0.91003007, 0.15246652, -10.409943], [-0.41296193, -0.10492031, -0.8399004, -16.2729],
                                 [0.8820865, -0.11257353, -0.45447016, -11.557314], [0.0, 0.0, 0.0, 1.0]],
                                dtype=np.float32)

        south12ego = south12ego[:-1, :]

        south12lidar = np.asarray([[-0.10087585, -0.51122875, 0.88484734, 1.90816304], [-1.0776537, 0.03094424, -0.10792235, -14.05913251],
                                   [0.01956882, -0.93122171, -0.45454375, 0.72290242], [0.0, 0.0, 0.0, 1.0]],
                                  dtype=np.float32)
        ...
        south22ego = np.asarray(
            [[0.650906, -0.7435749, 0.15303044, 4.6059465], [-0.14764456, -0.32172203, -0.935252, -15.00049], [0.74466264, 0.5861663, -0.3191956, -9.351643], [0.0, 0.0, 0.0, 1.0]],
            dtype=np.float32)

        south22ego = south22ego[:-1, :]

        south22lidar = np.asarray([[0.49709212, -0.19863714, 0.64202357, -0.03734614], [-0.60406415, -0.17852863, 0.50214409, 2.52095055],
                                   [0.01173726, -0.77546627, -0.70523436, 0.54322305], [0.0, 0.0, 0.0, 1.0]],
                                  dtype=np.float32)
    ...
    def create_folder(self, split):
        ...
        dir_list1 = ['point_clouds/s110_lidar_ouster_south_and_north_registered']
        for d in dir_list1:
